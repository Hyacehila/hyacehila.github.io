...
前面几篇文章里，我已经分别讲过三件事：[Reward 自己是怎样被生产出来的]({% post_url 2026-03-19-reward-design-evolution-from-rlhf-to-rlvr %})、[为什么 Agentic RL 真正重要的是训练闭环而不是单个算法]({% post_url 2026-03-21-from-sft-to-agentic-rl-training-loop %})，以及[真实系统里的数据治理、环境与 feedback stack 怎样长出来]({% post_url 2026-03-22-reward-and-training-in-agent-k-paperbench-amap %})。但如果你是从 Agent 进入这个方向，而不是从强化学习算法进入，读到这里通常还会卡住一个非常现实的问题：
...
在[前一篇关于 reward 演化的文章]({% post_url 2026-03-19-reward-design-evolution-from-rlhf-to-rlvr %})里，我已经说过很多次：reward 不是一个孤零零的总分，而是一条生产线。到了这个最小代码修复 Agent 里，这件事会变得更具体。

...
<p>The specific classification here is done by AI together with the design, and the result is to construct a local sub-classification with a secondary classification, with most areas having only a large structure. We got about 2w nodes, with some nodes connected by side; 10,000 nodes with a total margin of around 30,000; all one-way sides of the edge without attributes. The general information for individual nodes is the name of the nodes themselves, the descriptive text in the planner table, the classification of the metadata and the relevant information on the side of the connection. The margin itself is obtained from the process, not from models, determinative codes or manual labels.</p>

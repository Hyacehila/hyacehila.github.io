...
如果把这个判断再往前推一步，它会落到我在另一篇文章里提到的 `model-harness co-design`：模型差异不会只改变 benchmark 排名，也会改变工具命名、返回结构、自治边界、验证闭环和默认交互节奏的最优解。进入 agent 产品之后，模型与 harness 往往不是两套彼此独立的变量。

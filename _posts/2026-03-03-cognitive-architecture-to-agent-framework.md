...
从 2025 年开始，语言模型智能体逐渐成为了比基础模型更加火热的方向。无论是学术界还是工业界，都在围绕 Agent 展开全面的研究与工程实践。各式各样的 LLM Agent 被开发出来，用来解决曾经无法稳定交付的问题。

为什么需要构建语言模型智能体，不是本文要展开讨论的问题。这里只用一句话概括它的必要性：**LLM Agent 中的 LLM 为代理补充了关于世界的大量先验知识（prior）**，使它可以解决真实世界中的各类问题，而不再局限于极度受限的环境。

LLM Agent 本身并不神秘。**From Next Token Prediction to LLM Agent**，它的基本逻辑只是让模型从单纯的对话预测，变成一个可以通过对话、工具、环境反馈持续行动的代理。任何人只要了解语言模型 SDK 调用，懂得让模型输出可解析结构并执行工具调用，就已经在某种意义上开始开发 LLM Agent。

真正困难的是：构建一个 Agent 不能只依赖对业务流程的直觉。只靠个人直觉做出来的 Agent 更像玩具，而不是工具。开发者必须理解它的记忆如何组织、行动如何发生、反馈如何进入下一轮决策、权限如何被约束、失败如何被恢复。了解智能体的认知结构，认识框架应该如何设计，不是为了使用框架本身，而是为了构造一个更好、更能交付到生产环境的 Agent。

本文的核心判断是：**Agent Framework 的价值，不在于替开发者定义 Agent，而在于把 CoALA 中的记忆、行动、反馈、状态与权限管理，翻译成透明、可控、可调试的工程结构，去优化开发流程本身，而不是取代开发者的设计权限。**
...
Skills 这类轻量封装在这一阶段有吸引力，也是这两个产品很重要的组成部部分，也正是因为它不急着把能力塞进厚重框架。它把说明书、脚本、模板和参考资料打成一个渐进披露的能力目录，适合承载规则、流程和组织经验，却不强行假装自己已经是完整 runtime。关于 Skills 更详细的讨论，可以参考我的另一篇博客 [《从 MCP 到 Agent Skills》](/blog/2026/03/10/from-mcp-to-agent-skills/)。
...
首先，它应该提供**透明的状态管理**。上下文里有哪些内容，状态如何更新，检查点在哪里，任务如何恢复，都应该显式可见、可被审计、可被人理解。状态不透明，Agent 就只能停留在看起来会动的阶段。

其次，它应该提供**原生工具与协议接入**。MCP、函数调用、外部 API、文件系统、消息系统，乃至 Skills 这样的轻封装能力单元，都应该能被直接接入，而不是被迫再套一层框架自己的专有协议。好框架应该减少适配成本，而不是制造新的兼容层。

第三，它必须有**强可观测性**。Prompt 如何被组装，工具何时被调用，状态发生了什么变化，哪个节点触发了重试，整条轨迹是否可以 replay，这些都不该依赖框架黑盒 UI 去猜。Agent 不是传统后端服务，观测能力越弱，调试成本越高。

第四，**记忆应该成为基础设施**。无论是 episodic memory、semantic memory 还是 procedural memory，框架都不应假装自动帮你管好了。它应该把读写入口、检索策略、压缩、淘汰、冲突和 provenance 暴露出来，让开发者清楚记忆究竟如何影响当前决策。

第五，框架必须原生支持**human-in-the-loop 与权限边界**。审批点、人工接管、回滚点、危险操作确认，不应该靠业务方事后补洞。智能体一旦走进真实工作流，人类不只是兜底者，也是系统的一部分。想要完全替代人，通常只能把人的经验提前封装成 Workflow；对于自主决策 Agent，human-in-the-loop 仍然是可靠性的关键部分。

最后，它还应该对**agentic coding** 友好。随着模型编码能力继续增强，越来越多系统会让 Agent 直接阅读、修改甚至扩展自己的运行逻辑。框架越贴近原生语言、原生工程对象和原生工具链，就越容易被人和 Agent 一起理解；越是高层魔法式封装，越容易成为未来自动化自扩展的阻力。

归根结底，好框架的目标不是制造更高抽象，而是**减少认知债务，同时保留工程控制权**。它不是替开发者思考 Agent 应该是什么，而是在开发者已经理解任务结构之后，提供那些值得复用的基础能力。

未来框架不是更厚的抽象，而是更薄、更透明、更接近任务环境的基础设施。它应该让 CoALA 中的记忆、行动、反馈、状态和权限变得可见、可控、可替换，而不是把这些问题包进一个看似高级、最终必须被绕开的黑盒里。

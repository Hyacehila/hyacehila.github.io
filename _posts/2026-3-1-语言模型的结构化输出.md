...
以 Outlines 为例，其基础算法首先将开发者定义的 JSON Schema 转换为一个庞大的正则表达式（Regex），随后将其编译为FSM，映射得到绝对合法的集合，从而得到一个掩码向量$m(S_{t-1}, Regex) \in \{0, 1\}^{|V|}$。模型前向传播计算出下一个 Token 的原始对数几率向量（Logits）$\alpha \in \mathbb{R}^{|V|}$ 。

...
```mermaid
graph TD
    A[{"messages": [{"role": "user", ...}, {"role": "assistant", ...}]}] --> B[检测数据格式]
...
training_args = SFTConfig(
    ...
    assistant_only_loss=True,  # ★ 只对 assistant 回复计算 Loss
...
### 5.3 自定义 template 示例
本质上自定义的template就是在原始template的基础上，使用`{%- generation %},{%- endgeneration %}` 关键字包裹了助手生成的部分，从而使SFTTrainer能够通过这个关键字进行掩码。
...
| **Loss Masking** | 需要手动实现 | ✅ 自动处理 |
| **DataCollator** | 需要手动配置 | ✅ 自动配置 |

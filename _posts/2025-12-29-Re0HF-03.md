...
**本章将介绍 DPO（Direct Preference Optimization）**——一种让模型学习人类偏好的训练方法！
...
**直觉理解**：
- 如果策略模型比参考模型更倾向于生成 chosen 回答 → 奖励正
- 如果策略模型比参考模型更倾向于生成 rejected 回答 → 奖励负

**训练目标**：最大化 $(r_{\text{chosen}} - r_{\text{rejected}})$
...
| **数据** | (prompt, response) 对 | (prompt, chosen, rejected) 三元组 |
| **结果** | 能回答问题，但质量不一定最优 | 回答质量显著提升 |
| **时间** | 1-3 epochs | 1-3 epochs |
| **顺序** | 先进行 | **在 SFT 之后进行** |

⚠️ **重要：** DPO 应该在 SFT 之后进行！

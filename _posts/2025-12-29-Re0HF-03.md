...
$$R^*(x, y) = \beta \cdot \log\left(\frac{\pi^*(y|x)}{\pi_{\text{ref}}(y|x)}\right) + Z(x)$$

其中：
- $\pi^*(y|x)$ = 最优策略（我们想学到的策略）
- $\pi_{\text{ref}}(y|x)$ = 参考策略（SFT 后的模型，作为起点）
- $\beta$ = 温度参数，控制偏离程度
...
$$\mathcal{L}_{\text{DPO}} = \log \sigma\left(\beta \cdot \log\frac{\pi_\theta(y_w|x) \cdot \pi_{\text{ref}}(y_l|x)}{\pi_\theta(y_l|x) \cdot \pi_{\text{ref}}(y_w|x)}\right)$$

**这个损失的含义**：

- **分子**（我们想要的）：$\pi_\theta(y_w|x)$ 要大，$\pi_{\text{ref}}(y_l|x)$ 是基准
- **分母**（我们要避免的）：$\pi_\theta(y_l|x)$ 要小，$\pi_{\text{ref}}(y_w|x)$ 是基准

训练时，我们通过梯度上升最大化这个损失，使得：
1. $\pi_\theta(y_w|x)$ 相比参考模型上升 → 模型更倾向于生成 chosen 回答
2. $\pi_\theta(y_l|x)$ 相比参考模型下降 → 模型更不倾向于生成 rejected 回答

#### 2.3.5 为什么需要参考模型？

你可能会问：**为什么不能直接最大化 $\pi_\theta(y_w|x)$，最小化 $\pi_\theta(y_l|x)$？**

...
**但是**，DPO 的限制也很明确：它依赖离线的偏好数据（chosen vs rejected 对），不能直接利用可验证的奖励信号（如数学题的正确性、代码的测试结果等）。

**本章介绍 GRPO（Group Relative Policy Optimization）**——一种支持自定义奖励函数的强化学习方法。若要系统理解 GRPO 算法，仍应阅读原文；本文会补充论文中的预训练策略与部分理论讨论。

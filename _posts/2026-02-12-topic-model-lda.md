...
categories: [数据科学]
tags: [Graphical Models, Methodology]
author: Hyacehila
...
**主题模型 (Topic Model)** 基于这种直觉。它假设文档是由隐藏的“主题”生成的。作为该领域的经典模型，**潜在狄利克雷分配 (Latent Dirichlet Allocation, LDA)** 是一个**三层贝叶斯概率图模型**。它不仅用于自然语言处理，也是概率图模型（结合有向图、潜变量、共轭先验）的典型应用。

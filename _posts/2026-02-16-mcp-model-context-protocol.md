...
MCP 的目标是提供一个通用标准，让 AI 应用程序的开发和集成更简单、更统一。我们无须再为每个模型单独设计 Agent 代码，使用 MCP 构建的 Agent 可以不加修改地适配到所有支持 MCP 的 LLM 中。Anthropic 希望推动 LLM Tool Call 标准化（LLM 调用外部应用程序接口的标准）。

MCP 与视觉派代表两种 Agent 思路：前者依赖开发者提供接口适配，并统一这类接口；后者让 LLM 通过视觉识别看到接近人类看到的内容，再模拟人类操作。

接下来聊聊 MCP 为什么会出现，以及我们为什么需要它。MCP 是 prompt engineering 继续发展的结果。更结构化的上下文信息能明显提升模型的 performance；在构造 prompt 时加入场景信息，也能帮助模型理解真实场景中的问题。
...
- 由于 MCP 的选择基于 prompt，理论上任何模型都能适配 MCP，前提是你能提供对应的工具描述。

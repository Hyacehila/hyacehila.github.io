...
这类能力如果强行改写成 Tool Schema，会显得生硬和艰难；如果纯靠 Prompt，又会因为太长、太脆弱、太难维护而迅速失控（我不想并不是每个人都能写一个千字长文的提示词）。于是中间地带自然出现了：**不如把能力写成一个可以被读取、被展开、被分发的包。**
...
很多团队真正想分发的，并不是一个标准化 API，而是一整套做事方式：一段说明、一份模板、几个脚本、一些参考资料。MCP 并不是做不到这些，但 Skills 把这件事做得更轻：一个 `SKILL.md` 加几个目录，就已经足够开始共享。它在这里真正赢的不是更强的标准化，而是更低成本的打包和分发。

通过 `SKILL.md` 这个入口，提示词、操作说明和工作流第一次被当作可传播的工程工件来对待。对很多团队来说，这种分发方式比严格协议更接近真实需求，也更接近日常协作。也正因如此，今天回头看，Skills 赢的并不是只有它能打包说明、资料和脚本，而是它把这些原本分散在 MCP 协议、宿主 runtime 和 Tool Use 基础设施里的能力整理成了一个所有人都看得懂的目录。
...
**它不是新的统一协议，而是把说明书、参考资料、脚本和按需加载组织成默认体验的一种能力打包技巧。**

...
很多时候，大家会把 SFT 和 RL 写成一种此消彼长的关系，好像只要 online RL 足够强，前面的 demonstrations 就不重要了。但越看真实系统，越觉得这是一种单轮任务想象。对 agent 来说，SFT 压进去的不是抽象能力，而是具体的行为习惯：什么时候先读 observation，什么时候先调用工具，什么时候该追问，工具失败时是重试还是回退，什么时候应该停止继续探索并交付结果。
...
如果只保留一句话：**SFT 负责把模型送进可学习区域，offline shaping 负责把这个区域清干净。** 许多"后面 RL 训得很稳"的系统，不是突然学会了探索，而是前面已经把不必要的噪声和低级错误压掉了。

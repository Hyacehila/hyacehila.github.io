...
然而，**随着基础模型能力快速提升，以及 MCP（Model Context Protocol）的出现，Tool Calling 作为独立研究方向的价值正在减弱。** 当前前沿模型（如 GPT-4、Claude 等）已经原生具备较强的函数调用能力，不再总是需要额外微调或检索管线来实现工具使用。MCP 作为标准化的工具通信协议，则在工程层面处理工具描述、发现与调用的统一问题——它让工具使用从需要训练的能力，转向标准通信接口。

曾经困扰 Tool Use 研究的难题——上下文膨胀、工具检索、多轮调用——正在被更长的上下文窗口、更强的原生推理能力和标准化协议逐步消解。**学术界在 Tool Use 上的研究贡献已经走过了主要阶段，后续工作会更多发生在工程实践和协议标准层面，而非算法和模型训练层面。** 本文不再更新。

...
但应用统计学在现实中并没有因此停摆。很多时候，统计模型之所以还能用相关关系去说明某种“因果”，并不是因为模型真的从数据中自行识别出了因果结构，而是因为**因果关系早已内嵌在研究人员对问题的理解、研究设计与变量选择之中**。研究者往往并不是真的要从数据里“发现因果”，而是先基于领域知识对什么可能影响什么形成判断，再借助统计模型去估计强度、比较方向、量化不确定性。我们当然可以说，这并不是严格意义上真正由数据推出的因果识别，但这恰恰就是今天许多应用统计研究的现实：模型并不创造因果解释，它只是在尝试把研究者已经带入分析中的因果理解参数化、数量化。
...
在很多高维、复杂的现实问题中（如图像识别、推荐系统），人类根本无法预设正确的数学分布形式。机器学习摒弃了对形式完美的追求，通过交叉验证（Cross-Validation）等经验主义手段来评估模型的好坏，而非依赖理论上的渐进正态性。

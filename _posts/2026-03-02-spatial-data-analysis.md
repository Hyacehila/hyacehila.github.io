...
空间数据类型可以相互转化，用来反映不同问题。Fothringham 将连续数据分析中的 Kriging 模型和多边形数据中的 SAR 等方法整合到一个体系中。我们也可以将原本多边形数据中的发病数量弱化为是否发病并转化为点数据，或构造区域等值线转换为连续数据，借此换用不同方法分析
...
经典统计学中也会使用插值，但实际应用并不广泛；空间插值的应用则更常见，它用少量样本值推算更大范围上的空间属性值，实践价值很高

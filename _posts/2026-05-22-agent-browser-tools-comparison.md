...
author: Hyacehila
excerpt: 从 Playwright、Chrome DevTools MCP 到 Browser-Use、Stagehand、Skyvern、Lightpanda 和云端浏览器基础设施，梳理浏览器 Agent 工具如何围绕确定性控制、可读感知、行动编排和生产化承载逐步演进。
excerpt_en: "A discussion of how browser-agent tools evolved from deterministic automation scripts to browser infrastructure, covering Playwright, Chrome DevTools MCP, Browser-Use, Stagehand, Skyvern, Lightpanda, and cloud browser platforms."
featured: false
math: false
---
...
如果把 Playwright、Chrome DevTools MCP、Browser-Use、Vercel agent-browser、Stagehand、Skyvern、Lightpanda、Browserbase、Steel.dev、MultiOn 这些工具排成一列，很容易写成工具百科：每个工具一小节，讲功能、场景和限制。这样能帮人认名词，但解释不了一个更有意思的问题：这些工具为什么会这样演进？

我更愿意把这条线看成几个问题不断被拆开的过程：先让浏览器能被稳定控制，再让网页状态能被模型读懂，然后让行动可以缓存、验证和编排，最后把浏览器本身变成可承载的运行时资源。
...
重点不在于给工具排座次，而是看它们在改哪一层接口：从页面点击，到运行时诊断；从 DOM 解析，到模型可读表示；从自由探索，到缓存和工作流；从单机浏览器，到云端 session 和轻量 runtime。

## 第一阶段：从人写脚本到 Agent 调用浏览器

第一阶段的代表是 Playwright、Playwright MCP 和 Chrome DevTools MCP。它们不是典型的自主 Agent 框架，但它们是浏览器 Agent 工具链的起点。理解它们之间的关系，不能只看谁能点按钮，要先看底层那条线：CDP。
...
感知层决定模型是否看得清，行动层决定系统是否做得稳。工具演进不是从 A 替代 B，而是在这两层上不断改接口。

## 工具选型应该回到你卡在哪一层

所以选型不应该从“哪个工具最火”开始，而应该从“我的瓶颈在哪一层”开始。

如果你缺的是稳定控制，先看 Playwright 和 Playwright MCP。已知流程、E2E 测试、CI 回归，不需要强行交给自主 Agent。进一步说，如果目标是让模型像普通用户一样完成流程，Playwright MCP 更自然。
...
这样看，工具选型就不再是八选一，而是先定位你处在浏览器 Agent 工具链的哪个阶段：
...
Playwright 解决确定性控制，Chrome DevTools MCP 解决真实运行时反馈，但它们也会把大量浏览器信息带进 Agent 上下文。agent-browser 和 Browser-Use 解决模型可读的网页状态，Stagehand 解决开发者侧的混合接管和执行缓存，Skyvern 解决业务侧的视觉工作流，Lightpanda、Browserbase、Steel.dev、MultiOn 解决生产运行时和平台化。

所以这不是“AI 替代自动化脚本”的故事。更准确地说，浏览器被拆成了几层接口：可控制的环境、可读的状态、可验证的动作、可承载的运行时。

...
categories: [数据科学]
tags: [Data Curation, Statistical Inference, Methodology]
author: Hyacehila
...
本指南的目标是提供分享数据的最佳实践，避免从数据收集到数据分析过渡时常见的陷阱和延误。Leek 团队与众多合作者共事后发现，结果产出速度差异最大的因素，就是数据到达我们手中时的状态。根据我与其他统计学家的交流，这几乎是一个普遍真理。

我强烈认为，统计学家应该有能力处理任何状态的数据。查看原始数据、理解处理流程的每一步，并在数据分析中纳入隐含的变异来源，这些都至关重要。但对于许多数据类型，处理步骤已经文档化且标准化。因此，将数据从原始形式转换为可直接分析的形式，完全可以在寻求统计学家帮助之前完成。这样能显著加快周转时间，因为统计学家不需要先处理所有预处理步骤。

## 你应该向统计学家提供什么

为了加快分析速度，你应该向统计学家提供以下信息：
...
向统计学家提供你所能获取的**最原始形式**的数据至关重要。这能确保数据溯源贯穿整个工作流。
...
如果你对原始数据做了任何修改，它就不再是原始形式的数据了。**将修改后的数据作为原始数据报告，是导致分析过程变慢的常见原因**，因为分析师通常不得不对你的数据进行“取证研究”，以弄清楚为什么原始数据看起来很奇怪。（同时也请想象一下，如果新数据到了会发生什么？）

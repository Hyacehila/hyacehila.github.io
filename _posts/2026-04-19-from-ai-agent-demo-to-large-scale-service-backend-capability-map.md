...
这张表概括了本文要讨论的问题：后续的数据库、Go、并发、锁、队列、观测等话题，看似分散，实际上都指向同一个判断：**后端不是在给 Agent 做外围优化，而是在定义 Agent 能否成为服务。**
...
这不是架构师的审美问题，而是规模带来的现实分工。因为这些部分的流量模型、故障模型、延迟要求和扩展方式本来就不一样。Google Cloud 在 [Patterns for scalable and resilient apps](https://cloud.google.com/architecture/scalable-and-resilient-apps) 里把 `loose coupling` 和 `modular architectures` 放在靠前位置，并明确指出，独立服务可以分别发布、扩展和管理。分布式并不是什么“更高级的形态”，它更像是复杂性上来以后系统被迫暴露出的自然边界。
...
这一章要建立的直觉是：**服务拆分解决的不是优雅，而是把不同故障模型和扩展需求隔开。**
...
许多 Agent 产品先出问题，不是因为模型不够强，而是权限、安全或成本先失控。
...
这张表也解释了为什么后端学习笔记这个系列会长成接下来的样子。后面学的每一个主题，都不是孤立知识点，而是在回答全景图里的某一个缺口。
...
但如果你想把它做成一个大规模服务，问题中心一定会转移。重要的会变成：
...
**从 Demo 到服务，Agent 最大的变化不是更会推理，而是开始被后端接管。**

...
这不是怀旧，也不是树模型的历史残留。更准确地说：**在大量结构化表格任务上，提升树至今仍是最常见的强 baseline，而且在不少场景里依然是最优选择或接近最优选择。** 它们长期不退场，不是因为业界保守，而是因为仍然有效。
...
在这种情况下，一个带强归纳偏置、参数控制清晰、对中小样本更友好的提升树，往往更容易给出稳定结果。**不是所有任务都大到必须靠重负载的表示学习才能解。**
...
提升树之所以长期强势，也因为它们在这些维度上形成了成熟生态。团队需要的常常不是理论上最先进的真实SOTA，而是今天就能稳定把任务处理好。
...
原始 GBDT 很重要，因为 XGBoost、LightGBM 与 CatBoost 都站在这条主线上。但在今天的工程实践里，决定体验和性能的，往往不再只是Boosting 这个抽象思想，而是这些现代实现如何处理目标函数、分裂查找、缺失值、类别特征、内存布局与并行效率。
...
观察表格学习实践，会看到一个稳定现象：当任务还没有被完全摸透时，人们往往愿意先试一个 XGBoost。原因并不神秘，它最吸引人的从来不是某个指标偶尔特别亮眼，而是整体上的平衡感。

它足够强、足够稳，目标函数灵活，超参数逻辑相对清晰，缺失值处理也自然。XGBoost 提供的常常不是最终答案，而是一个**可靠的Baseline**：先用它把问题结构摸清楚，再决定是否更激进地追求速度、类别特征或某些专门优化。

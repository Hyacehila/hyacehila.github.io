...
把 Agent 变成可交付系统的，通常不是核心 loop 本身，而是围绕 `LLM` 不确定性搭出来的一整套外围工程。核心循环当然重要，但它往往只是最短的一段。

最长、也最贵的一段，通常是把这个循环关进一个可控系统里。重点不是让它更会想，而是让它在想错、说错、调错工具、上下文漂移、连续运行几十轮之后，系统仍然不至于一起失控。

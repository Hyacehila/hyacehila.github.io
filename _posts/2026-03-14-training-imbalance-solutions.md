...
所以，样本不均衡难处理的地方，从来不只是“类频数不平衡”，而是**训练分布、损失设计、概率估计、阈值决策、部署先验**经常同时错位。统计学习领域很早就意识到这一点，长尾深度学习则把问题进一步拆到表示学习、分类头偏置、后验修正与校准层面。把这些脉络放在一起看，更容易判断自己该先改数据、改损失，还是先改评价方式。
...
这三件事重叠，但不是一回事。比如医学风险预测里，正例少只是表象，难点是**你需要一个可解释、可校准的概率输出**；而在长尾图像识别里，不少工作追求的是 tail class 的 top-1 / macro accuracy，此时比起概率解释，表示学习与分类头偏置反而更关键。
...
但 SMOTE 有效的前提，是**少数类在局部邻域里确实有可插值的流形结构**。如果少数类本来就夹在多数类里面，或者少数类样本带有标签噪声，那么合成样本很可能只会把边界区域进一步污染。Prati 等人（2004）对“类别不均衡 vs 类别重叠”的分析提醒我们：难的往往不是频数，而是几何关系。

从今天回看，Fernandez 等人 2018 年的 15 周年回顾也给出了很公允的判断：SMOTE 之所以长期成为 baseline，不是因为它万能，而是因为它在“样本很少、模型容量有限、少数类局部结构还算稳定”的任务里，确实是一个低门槛、可解释、常常有效的起点。

### 2. 代价敏感学习：先改错误成本

Elkan（2001, IJCAI）对代价敏感学习的讨论，到今天依然值得重读。他的观点很朴素：**如果漏报和误报的代价不同，那么默认 0.5 阈值就没有任何天然正当性。**

这条线给实践者的启发是，通常你不需要先改数据，而是应该先把损失函数或决策阈值改成更符合业务代价的形式。比如欺诈检测里，漏掉一个高风险交易的代价，可能远高于多审查几个正常交易；医学筛查里，早筛阶段往往更愿意接受较高误报来换更高召回。
...
这类方法今天依然值得记住，尤其是当你处理的是**小样本、表格数据、基学习器不算太强**的任务。因为在这些场景里，深度长尾方法的收益并不会稳定迁移到这些场景，反而是集成 + 合理采样更稳。

...
在前文[《从智能体的认知结构到智能体框架》](/blog/2026/03/03/cognitive-architecture-to-agent-framework/)中，我借 CoALA 讨论了智能体的认知结构，并留下一个判断：**Agent 的工程发展史，很大一部分是在用外部系统弥补当前 LLM 在记忆和推理上的短板。**

[《从 Working Memory 到长期记忆：Agent Memory 的全景图》](/blog/2026/03/21/agent-memory-panorama/) 主要回答的是"Memory 在研究上已经长成了什么样子"；这篇文章换一个问题：**当我们实现一个 Agent 或一个 Agent 框架时，应该怎样把这些想法组织成可运行的 Context 系统？**

Context Engineering 和 Memory 高度交叉。按照 CoALA 的理解，Context Engineering 可以看作对 **Working Memory** 的工程化管理：在有限的上下文预算里，决定什么该进场、什么该留在场、什么该退场，以及什么应该在需要时被重新召回。只要底层还是 attention 机制和有限窗口，这四个动作就始终支撑着 Agent Context Engineering。

所以这篇文章不再承担文献综述职责，也不再把论文拆成附录逐篇解释。这里更关心工程面：**系统应该如何分层、Memory Manager 应该承担什么职责、读写路径应该如何组织、遗忘和隔离怎么做，以及我们准备参考哪些技术来把这件事放进框架。**
...
Context Engineering 不是把一堆 memory 论文硬拼进框架里，而是要把它们翻译成系统问题：存储层怎么组织，读路径怎么走，写路径怎么走，哪些策略靠规则，哪些策略将来可以学习，哪些动作必须能被开发者检查和干预。
...
- **审计**：记录来源、时间、冲突状态和操作日志，保证可回溯。
 ...
 Memory 是基础设施，不是零散 prompt 技巧。MemGPT 提醒我们要把上下文预算当成操作系统问题来处理；Mem0 说明抽取、更新和召回可以被包装成独立 memory layer；Memory-R1 提供了 learned CRUD 的方向；ACE 和 Dynamic Cheatsheet 则说明 Working Memory 不只能压缩，还可以在任务执行中通过 delta update 持续演化。对工程系统来说，重点不是把这些论文逐篇搬进来，而是吸收它们各自证明有效的机制。
...
比如"用户喜欢拿铁"与"用户最近开始戒咖啡"就不是单纯的二选一覆盖问题。系统需要知道两条记忆的时间关系、适用条件和冲突状态，而不是把它们合并成一条失真的静态偏好。
...
如果把上面的讨论整理成一个可实现的工程方案，我现在更倾向于按下面五层落地：
...
这里面最重要的一点是：我们不会把 Context Engineering 只做成一个检索外挂。它必须同时覆盖读、写、更新、遗忘和审计，否则只能解决“找资料”这一小块问题。

## 结论：Context 管理的核心矛盾与下一步

回头看，Context Engineering 落地时至少要检查三件事：

- 信息是否够用，同时没有把注意力预算和调用成本吃光。
- 规则是否足够稳定，同时允许经验和用户状态继续更新。
- 自动写入是否可审计、可删除、可回滚，而不是把 memory plumbing 藏到黑盒里。

所以，Context Engineering 的重点不是单一技巧，而是把这些判断纳入一个可治理的系统设计里。更强的 Agent 不只是窗口更长。它要能说明哪些信息进入当前上下文，哪些被降级或删除，哪些被写入长期记忆，以及这些决定为什么发生。

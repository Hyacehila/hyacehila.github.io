...
这个系列从基础设施建设开始，经过认知架构、上下文工程、MCP、Skills，一路讨论到 RL Agent 与 LLM Agent 的转向。回头看，每一篇都绕着同一个问题展开：Agent 如何与外部世界交互。

MCP 回答协议互通，Skills 回答能力封装，认知架构回答决策循环。但还有一个问题一直悬着：Agent 运行的那个“外部世界”本身，是否需要一个统一的抽象？

这不是一个假想的问题。在 RL Agent 那篇里我们讨论过，纯 RL 在开放世界撞墙的根因之一就是环境太复杂、奖励太稀疏、状态转移不可控。LLM Agent 继承了更强的先验知识，但并没有自动解决环境接口碎片化：浏览器是一套，终端是一套，benchmark 又是一套，训练和部署也常常割裂。

最近注意到一个项目正在尝试回答这个问题：[AEnvironment](https://github.com/inclusionAI/AEnvironment)。它的核心主张是 **Everything as Environment**。

这篇文章不是项目推广。我想借 AEnvironment 这个具体案例，讨论统一环境层是否踩中了真实痛点，以及它目前能做到什么程度。
...
Tool calling 的默认假设是世界可以被抽象为一组函数。这在很多业务自动化里完全成立，但在浏览器任务、终端任务、benchmark 任务这类场景下，这个假设开始松动：
...
如果回到 RL Agent 的经典设定，这并不难理解。RL 的循环就是 `observation -> action -> reward -> next observation`。这个循环天然要求一个有状态的、可持续交互的环境。LLM Agent 虽然把动作空间从连续向量变成了自然语言，但只要任务足够复杂，它面对的交互结构和 RL 是同构的。
...
在 RL Agent 那篇里我们讨论过 The Second Half 的洞察：**当模型已经具备泛化能力后，瓶颈转向了 Evaluation。** 而今天的现实是：benchmark 一套代码、线上部署另一套代码、RL 训练又要自己搭环境。同一个 Agent 在 SWE-bench 上跑的环境和在生产中跑的环境完全割裂，导致评测成绩难以迁移到真实任务。
...
AEnvironment 和 MCP 不是竞争关系，而是分层关系。用一个不太精确但好理解的比方：**MCP 解决的是接口标准，AEnvironment 想解决的是整套设备怎样接进工作流。** AEnvironment 的 README 明确写到它是"通过扩展标准化 MCP 协议"来提供环境基础设施的——它不反 MCP，反而需要 MCP。
...
AlphaProof 和 AlphaEvolve 之所以成功，恰恰是因为它们有近乎完美的验证器——Lean 证明系统、自动化测试套件。在这些领域，奖励信号是清晰的、即时的、可规模化的。AlphaProof 在 2024 年 IMO 上拿到银牌水平，重点不在于 RL 有多强，而在于**一旦高质量环境反馈存在，经验学习就可能迅速超越纯人类数据路线**。
...
**这也是统一环境层的更深层价值**：如果 benchmark 和运行时共享同一套 Environment interface，那么奖励信号的定义、验证和迭代就可以在统一的框架下进行，而不是每个场景各搭一套。环境层不只是 agent 的运行容器，更是奖励信号的生产车间——环境设计得越好，reward 就越清晰，agent 的学习就越有方向。
...
更关键的是动作空间的统一。NitroGen 没有为每个游戏定义一套动作，而是把超过 1000 个游戏统一映射到同一个 gamepad action space：**16 维二值按钮向量**（d-pad、face button、肩键、扳机等）加上 **4 维连续摇杆向量**，观测则统一为 RGB 帧。丢掉了游戏特异性，但换来了跨游戏迁移、共享预训练、统一 benchmark 的可能。最终他们在 10 个商业游戏、30 个任务上建立了评测基准。
...
[BrowserGym](https://github.com/ServiceNow/BrowserGym) 只做了 web 一个领域的环境统一（WebArena、MiniWoB、WorkArena），已经够复杂了。NitroGen 只统一了 gamepad 游戏这一个子集，也需要构建从视频数据管线到模拟器封装的完整工程栈。AEnvironment 想做全部——这个野心让人尊敬，但也让人担心。历史上，试图统一一切的抽象层往往要么变得过于臃肿，要么在困难场景下退化为最小公倍数的妥协。
...
这里有一个微妙的区分：AEnvironment 的目标用户可能主要不是应用开发者，而是 RL 研究者和 benchmark 维护者。对前者来说更轻是主要诉求；对后者来说更统一才是主要诉求。如果 AEnvironment 能找准自己的用户画像，这个矛盾也许可以缓解。但目前的 README 给人的印象是它想同时服务所有人——这往往是危险的。
...
从 RL Agent 到 LLM Agent 的转向中，环境始终是绕不开的要素。纯 RL 在开放世界撞墙的教训告诉我们，Agent 能力不只取决于模型有多强，也取决于环境接口是否清楚、可复现、可评估。"Everything as Environment" 这个说法有价值，因为它命名了一个已经存在但经常被忽略的层次。

### 但项目还缺外部验证

作为蚂蚁内部 Agentic RL 训练闭环的配套，AEnvironment 的定位是清晰的。但如果把它看成通用 Agent 环境层，它还缺外部验证：独立论文、大规模外部采用、长周期生产经验，目前都还不充分。

AEnvironment 现在最值得看的，可能不是代码本身，而是它代表的抽象方向。BrowserGym 在 web 领域示范了环境化，NitroGen 在游戏领域证明了环境统一后可以支持跨域预训练，AEnvironment 则把同一个问题推到更宽的 Agent 场景里。至于最后由谁实现这层抽象，是 AEnvironment 自己、Gymnasium 的演化、MCP 的扩展，还是某个还没出现的项目，现在还无法判断。

...
所以“集成学习”不只是一个工具箱标签，也是一套理解模型关系的方法。树模型文章里看到的 Random Forest 与 GBDT，只是这套框架里的两个典型分支。

这一节的重点，不是背多少算法名字，而是先看 ensemble **如何制造多样性**，再看它**试图解决偏差还是方差的问题**。这两个坐标定住后，后面的方法就容易放回同一张地图里。

## 从最简单的融合开始：Averaging 与 Voting

很多时候，集成学习不是从复杂框架开始的，而是从最朴素的问题开始：**如果我已经有了多个模型，能不能直接把它们合起来？**
...
它不再依赖“并行生成多个模型然后平均”，而是让基学习器按顺序串起来。后一轮学习器的任务，不是从零开始重新学一遍，而是重点处理前一轮还没学好的部分。

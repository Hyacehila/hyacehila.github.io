...
for (const name of fs.readdirSync(posts).filter(name => name.endsWith('.md'))) {
  ...
  if (fm.categories?.[0] === 'Programming') {
...
// Drafts are Chinese-only; directory membership controls publication.
for (const file of walk(path.join(root, 'source', '_drafts')).filter(file => file.endsWith('.md'))) {
  const fm = frontMatter(fs.readFileSync(file, 'utf8'));
  if (!fm.permalink) continue;
  const route = fm.permalink;

...
tags: [Builder Mindset, Product Thinking, Software Engineering]
excerpt: "摘要"
...
- **隐藏首页**：给文章加 `hidden: true`，它就不再出现在 Home 文章流；`/archives/`、分类、标签仍保持全量索引，单篇链接也不变。
...
- **i18n / taxonomy 校验**：提交前运行 `npm run check:i18n`，检查 `data-i18n` key、文章英文标题/摘要、两级分类、标签规范、英文默认 UI 中文残留和 CJK permalink 映射。

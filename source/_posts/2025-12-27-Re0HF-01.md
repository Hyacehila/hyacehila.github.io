...
title_en: "Re0-01: HuggingFace Transformers Trainer"
date: 2025-12-27 22:00:00 +0800
categories: ["Foundation Models", "Training & Alignment"]
tags: ["Hugging Face", "Fine-Tuning", "LoRA", "Loss Masking", "Tutorial", "Series"]
author: Hyacehila
hidden: true
excerpt: 详解使用 Transformers Trainer微调LLM：Loss Masking、量化与LoRA
excerpt_en: "A detailed guide to fine-tuning LLMs with Transformers Trainer, covering loss masking, quantization, and LoRA."
permalink: '/blog/2025/12/27/Re0HF-01/'
---


这篇文章中的问题也可以和[Re0-02 : HuggingFace TRL SFTTrainer](/blog/2025/12/28/Re0HF-02/)、[Re0-03 : HuggingFace TRL DPOTrainer](/blog/2025/12/29/Re0HF-03/)放在一起阅读，以比较相近的概念如何在不同语境中展开。
...
预训练的大语言模型已经读过大量公开文本，但直接使用时仍会遇到这些边界：

- 它可能不知道你公司的内部规定
- 它可能不了解你特定领域的专业知识
- 它可能不会按照你期望的格式回答问题

**微调（Fine-tuning）** 用任务数据约束模型行为，让输出更贴近具体场景。
...
| 组件 | 作用 | 本教程使用 |
|------|------|-----------|
| `transformers` | 加载和使用预训练模型 | [是] |
| `datasets` | 加载和处理数据集 | [是] |
| `peft` | 参数高效微调（LoRA等） | [是] |
| `bitsandbytes` | 模型量化（4-bit/8-bit） | [是] |
| `trl` | 强化学习和高级训练 | 下一章 |
...
常见判断是：**"模型已经量化成 4-bit 了，训练时应该不需要很多显存才对"**

但量化主要减少模型权重的**存储空间**，训练时还要存储权重之外的内容。
...
>
> 想要有效降低训练显存，必须减少可训练参数的数量 → 这就是 LoRA 的作用
...
注意 **重要**：SFT 必须使用 `DataCollatorForSeq2Seq`，因为它尊重你预处理好的 `labels`（包含 Loss Masking），`DataCollatorForLanguageModeling` 则不尊重。
...
如果你使用 `uv` 管理环境，可以在 `code/` 目录自建 `pyproject.toml`；注意 `torch` 与 CUDA 版本的匹配。
### 6.2 运行训练
...
| **DataCollator** | 组合样本成 batch，是本节除了 Loss 以外需要重点理解的组件 |
...
- [Transformers 文档：Trainer](https://huggingface.co/docs/transformers/main/en/main_classes/trainer)
- [Transformers 文档：量化（bitsandbytes / 4-bit / 8-bit）](https://huggingface.co/docs/transformers/main/en/quantization)
- [PEFT 文档：LoRA](https://huggingface.co/docs/peft/main/en/conceptual_guides/lora)

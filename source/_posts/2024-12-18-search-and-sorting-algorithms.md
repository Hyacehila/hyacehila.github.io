...
红黑树（red-black tree，RBT）是另一种自平衡二叉搜索树（self-balancing binary search tree）。它和 AVL 树的共同目标都是维持二叉搜索树的有序性质，并把查找、插入和删除的时间复杂度控制在 $O(\log n)$；区别在于，AVL 树直接限制左右子树的高度差，而红黑树通过颜色约束间接控制整棵树的高度。RBT 的宽松方案可以换来对插入和删除的常数水平时间复杂度优势，并带来轻微的查找性能下降。

...
把平衡调整放进插入和删除的递归回溯过程后，每层结点都会更新高度并检查平衡因子。AVL 树的查找、插入和删除时间复杂度都能保持在 $O(\log n)$。AVL 通过控制树高解决了内存中的退化问题；当数据规模很大、结点主要存放在磁盘上时，还需要进一步减少每层访问的外存次数。下一节介绍的多路搜索树让一个结点保存多个关键字和孩子，正是沿着“降低树高、减少访问次数”的方向继续发展。

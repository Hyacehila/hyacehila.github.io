...
tags: ["Language Agents", "Reinforcement Learning", "Open-World Agents", "Uncertainty Modeling", "Memory", "Evaluation"]
author: Hyacehila
...
excerpt_en: "The shift from RL agents to LLM agents is not a simple model replacement; learning signals move from numeric spaces into language spaces, reshaping priors, evaluation, and memory."
...
Large Language Model 的诞生，让机器获得了理解自然语言、处理视觉信息、综合信息思考的能力。人机交互也随之换了一种入口：旧时代的操作系统把人的模糊指令翻译成精确的计算机指令，而自然语言可能会成为新时代的操作系统语言。LLM Agent 则像这个操作系统的早期雏形。
...
语言模型 Agent 改变了问题入口。LLM Agent 和传统智能体面对的是不同的输入空间和 Action Space；前者的空间更大，也更难直接收敛。但自然语言和图像预训练带来的先验知识，让 Agent 不必完全依赖环境里的盲目试错，也让不确定性可以被更细地描述出来。

从 RL Agent 到 LLM Agent，不只是“旧模型被新模型替代”。更重要的变化是：学习信号从数值空间迁移到语言空间，泛化机制也从环境内的盲目试错，转向预训练先验、推理、反馈与记忆的共同作用。

The First Half 关心“如何获得一个泛化模型”，The Second Half 则追问：“如何让这种泛化能力在真实世界中被正确评估、正确使用，并沉淀为持续进化的智能体系统”。
...
这也是为什么分层强化学习长期以来被寄予厚望，却始终难以在开放世界上彻底破局。问题不只是“层次不够深”，还在于**纯 RL 系统缺乏原生的符号化抽象与语义规划能力**。它很难从环境反馈中直接长出“如果要资源 B，就必须先有工具 A”这样的结构化知识。于是，开放世界暴露了 RL Agent 的边界：它不是不能学，而是当任务需要常识、抽象规划和语义压缩时，仅靠环境内的数值反馈并不够。

## The Second Half

本节讨论从语言模型到语言模型智能体，主要基于 Shunyu Yao 的 The Second Half，重点看 LLM Agent 的产生，以及新时代里更值得追问的问题。
...
只有 RL 能通向 AGI、实现可能存在的完全泛化模型——这是一个长期存在的判断。从 21 世纪初的 DeepBlue，到 AlphaGo / AlphaZero，再到 OpenAI Five，这些都是对 RL-AGI 的尝试，并且确实取得了不错的效果。但在 OpenAI 的尝试中，哪怕构建了完善的 Environment 让 RL 去学习，RL 可以解决不少问题，却始终无法泛化到“能够使用整个计算机”的能力——it does not work.
...
因此，接下来更可能发生的不是 RL 消失，而是职责重排：RL 退回低层控制、连续动作和局部优化，LLM 主导高层规划、语义推理、工具/代码生成与记忆协同。The Second Half 开始之后，Agent 的上限会越来越取决于环境、评估、反馈工程与外部记忆如何被组织到同一个系统里。

## 附录：Agent Dev 工业界实战指南

无论是 Plan4MC 对多模态奖励的处理，还是 Voyager 将代码作为策略空间的做法，它们都给工程落地留下了几条可复用的线索：动作空间要分层，环境反馈要可读，记忆库不能只做日志堆积。这些内容不是本文主线，但很可能是工业场景最先遇到的问题。

### 动作空间解耦架构（Action Space Decoupling）

在复杂智能体系统的落地中，强行要求单一 LLM 完成从意图理解到像素级控制的全栈任务是低效且高风险的。“动作空间解耦”是当前最成熟的设计原则：

- **认知与逻辑隔离**：任务的意图识别、复杂依赖拆解与长程规划交给 LLM 或多智能体协作框架；底层易出错的指令序列不要直接让 LLM 逐步输出。
- **代码 / 脚本处理确定性任务**：如果工业环境已经有稳定 API，例如云控制台、Web DOM、终端 CLI，就让 LLM 生成 Python 或脚本去承载循环、异常捕获和控制流。
- **RL 处理非结构化物理控制**：只有在缺乏精确 API、又需要高频传感器闭环反馈的场景里，才把强化学习策略网络放到底层控制中。LLM 更适合下发高层语义指令。

### 环境观测的文本化与反馈工程（Observation Textualization & Feedback Engineering）

既然 LLM 的原生模态是自然语言，那么环境观测接口的设计质量就直接决定了 Agent 的智力上限。

- **高维状态需要语义降维**：不要把未经处理的数组矩阵或杂乱日志直接扔给 LLM，而要用 parser 把剩余资源、节点健康度、拓扑结构等状态翻译成结构化 JSON 或精炼文本。
- **闭环反馈要足够细**：当环境执行报错时，底层执行器不应该只返回 `False`，而要回传 stack trace、缺失依赖、资源差值等上下文。模型能否修正逻辑，很大程度上取决于这些反馈文本是否可用。

### 基于记忆库的持续演进架构（Memory-based Continuous Learning）

微调成本高，也可能引发灾难性遗忘。对多数工程场景来说，为智能体挂载非参数化记忆系统，是更容易维护的持续学习路径。

- **经验外置化与 RAG 检索**：系统记录解决特定业务问题的成功思路、有效 API 调用模板和验证过的代码片段，再把它们组织成可检索的领域技能库。
- **自我验证与沉淀机制**：写入记忆库的数据必须先被证实有效。只有当执行逻辑真实改变了环境状态，并完成既定子目标时，这段工作流才值得沉淀。
- **技能组合**：新任务规划时，模型应主动检索记忆库，并像调用标准库函数一样组合历史技能。业务经验积累越多，系统越有机会从“机械执行”走向“复用旧经验解决新问题”。

综上，大语言模型的先验知识与非参数化记忆，为长程、开放世界任务提供了一条更现实的工程路线。它不是把传统强化学习整体替换掉，而是把高层规划、环境反馈、工具执行和经验沉淀拆开，让每一层使用更适合自己的方法。

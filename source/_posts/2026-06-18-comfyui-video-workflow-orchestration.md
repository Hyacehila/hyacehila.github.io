...
只是视频麻烦得多。图片抽卡失败，最多是这一张不行；视频失败，可能是角色第一秒还对，第三秒脸就漂了，也可能上一段和下一段接不上，镜头运动不错但产品变形了。当前不少视频模型仍以短片段、单镜头、文生视频或图生视频为基本单位。比如 Stable Video Diffusion 发布时提供的是 14 帧和 25 帧的图生视频模型，更强的闭源模型如 Seedance 2.0 和 HappyHorse 也最多支持 15 秒。可人们需要的显然不止 15 秒。
...
我的判断先放在这里：ComfyUI 已经从生图 UI 变成了开放的生成式 workflow/runtime；视频生成正在把问题推向更高一层。它值得补一些镜头、分段、批量抽卡、核验和长任务管理能力，但不必变成 LibTV。ComfyUI 最有意思的地方，还是开放、可控、本地化、模型生态和节点可组合。
...
现在的 ComfyUI 可以理解成三个东西叠在一起：可视化生成图谱、本地/远程推理运行时、可被程序调用的 workflow 系统。它最早因生图而流行，但现在讨论它，只看图片已经不够了。

这里还要补一层生图 Pipeline。前面的视角主要围绕本地模型拆解展开，现在很多人也会通过外部 API 和 Partner Nodes 生成图片。一个基础 Pipeline 往往先用快速模式区分风格、批量抽卡，在 4 到 10 张多样性图片里挑选，再切到更高质量的模式做多轮微调。系统还需要自动化评估，常见做法是引入外部 LLM Rubric，减少人工反复写提示词，把人留在真正需要判断的时候。
...
长视频的问题已经从生成问题变成了系统问题。模型能力当然重要，但素材结构、执行顺序、筛选机制和失败重试同样重要。没有这些外部结构，模型再强，也很容易停在一个不错的短片段，难以变成可以交付的产品（如短剧）。
...
这和 ComfyUI 的抽象层级不同。ComfyUI 的节点更像推理算子：加载哪个模型，使用哪个条件，如何采样，如何解码，如何把结果传给下一个节点。LibTV 的对象更像创作资产：项目里有哪些角色，哪个镜头要生成什么，参考图是什么，当前会话进展到哪里，结果如何下载，素材如何继续被使用。两者的不同也来自生成模型所处的阶段：前者来自开源生图模型的高峰期，后者则是利用闭源生成模型做长视频的工具。
...
中层是 orchestration。它负责镜头、场景、批量候选、核验、资产继承、长任务和版本。它可以是视频生成的专门层；生图和视频生成分别演进，再通过一个资源层共享角色、产品、场景和风格资产。这里只是在聊一个方向，具体怎么做还得看 ComfyUI 的生态会长成什么样。

上层是 App Mode、Agent 或 Studio UI。普通用户不需要看到所有节点，只需要上传素材、填写需求、选择候选、确认结果。复杂 workflow 可以被封装成应用，也可以被 agent 调用。

真正有用的系统，可能会同时吸收两边的经验：底层像 ComfyUI 一样开放可控，上层像 LibTV 一样围绕视频项目组织。视频生成的兴起会逼这类 runtime 面对更麻烦的问题：怎样把一次次短片段生成，组织成可以被创作、筛选、继承、重试和交付的长视频生产流程。

也许未来的视频模型会强到一次生成完整短片。但在那之前，工作流编排仍然绕不开。哪怕模型最终可以生成几分钟甚至几个小时的短片，真正有价值的长视频生产仍需要一层理解镜头和项目的结构，而不是给出一句 prompt 后把剩下的事全交给模型猜。

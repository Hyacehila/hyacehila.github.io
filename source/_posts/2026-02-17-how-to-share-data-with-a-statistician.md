...
我认为，统计学家应该有能力处理任何状态的数据。查看原始数据、理解处理流程的每一步，并在数据分析中纳入隐含的变异来源，这些都很重要。但对于许多数据类型，处理步骤已经文档化且标准化。因此，将数据从原始形式转换为可直接分析的形式，完全可以在寻求统计学家帮助之前完成。这样能显著加快周转时间，因为统计学家不需要先处理所有预处理步骤。
...
向统计学家提供你所能获取的**最原始形式**的数据很重要。这能确保数据溯源贯穿整个工作流。

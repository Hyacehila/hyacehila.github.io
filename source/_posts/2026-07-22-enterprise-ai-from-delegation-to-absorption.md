...
[微软 2026 Work Trend Index](https://www.microsoft.com/en-us/worklab/work-trend-index/agents-human-agency-and-the-opportunity-for-every-organization) 比较接近第一类。它结合 Microsoft 365 的匿名生产力信号，以及对 10 个国家 2 万名 AI 用户的调查，问的是员工开始会用之后，组织有没有跟上。报告里的 Transformation Paradox 很具体：65% 的 AI 用户担心自己跟不上变化；与此同时，45% 的人觉得，与其花时间用 AI 重做工作，不如先完成眼前目标更安全。只有 13% 的人认为，就算短期结果没达成，自己也会因为尝试用 AI 重做工作而得到认可。
...
模型直接覆盖的，只是其中一部分。它能理解文本、生成候选、调用工具，有时还能连续执行。剩下那些环节不会随着模型升级自动补齐。许多项目停在试点，原因就在这里，从 demo 到系统还有很遥远的距离。
...
AI 系统会把这种关系放大。一个生产级系统同时碰业务目标、知识与数据、模型与评测、工具调用、权限、成本和安全。MLOps 把持续集成、交付和训练放在同一条工程链路里，也是为了让这些交接可以连续发生。组织里每多一个模糊的 handoff，系统里往往就多一层接口、审批或等待。
...
微软关于 AI CoE 的指引有相近的意思：AI 能力通常要建在已有的云、数据和治理团队之上，而不是另立一个只负责“做模型”的孤岛。[Microsoft AI CoE 指引](https://learn.microsoft.com/en-us/azure/cloud-adoption-framework/ai/center-of-excellence) 对开发者来说，这意味着系统里的 owner、输入输出契约、权限边界、SLO、评测和升级路径，最好能和团队真实的协作关系对上。工具调用链一旦又长又脆，可以先检查团队之间是否也有高频、模糊、没人负责的交接。
...
企业 AI 的成本不只有 token、GPU、SaaS 席位和供应商报价。还包括把一段工作重新变清楚、接得进去、查得出来、出了问题有人负责的成本。
...
企业做 AI 时，很容易按“看起来聪不聪明”给任务排优先级。能写战略报告的模型，似乎应该比会分拣工单的模型更有价值。落地顺序往往相反。
...
斯坦福把成功样本里的人机分工分成了几种。全自主的 agentic 实现只是一部分；更多项目让 AI 处理大部分常规工作，人只看例外，或保留关键输出的人类审阅。报告观察到，高量、可恢复的任务里，“AI 处理常规 + 人审例外”可以带来很高收益；临床文书、对外内容、高风险决策和复杂编码则仍需要协作与审核。

自动化程度不是产品成熟度的一根刻度尺。

让人逐条审批所有模型输出，速度当然会慢；但任务风险高、上下文隐性、错误不可逆时，硬追求全自动，只是把审查从流程里挪到事故之后。一个做得好的例外升级系统并不保守。它要识别常规路径、设阈值、保留证据、安排人工接管，再把人工判断送回下一轮。

决定先做 Copilot、审核辅助还是更高自动化前，先问这五件事：

- 常规部分有多少，是否稳定重复？
- 系统能不能识别成功和失败，还是只能靠资深员工凭感觉判断？
- 出错后能回滚或补救吗，还是会直接影响客户、资金、合规或品牌？
- AI 拿得到完成任务所需的系统状态吗，还是只能根据一段零散文本回答？
- 异常发生时，有没有明确的人、队列和时限接手？

而对于AI 应用开发者，他们最有价值的能力，常常是一种将智能与人工互相翻译的能力：把业务人员说不清的工作，翻译成模型能参与、系统能执行、人能验收、组织能承担的流程。强模型嵌进界面当然也能做出好产品；客户真要把它放到核心工作里，剩下这些部分才会决定产品能不能留下来。
...
但这里有一个非常单独的问题，AI 带来的速度不一定仅仅是成本优势。如果读者玩过类似 LOL 之类的 MOBA 游戏，他们会知道移动速度是这个游戏最关键的属性，他带来拉扯的能力。速度快本身就是一种优势，他会将不可能变成可能，而如何利用这种可能是决策层需要考虑的。

节省的时间有没有变成业务结果？人被转去做什么？客户得到的服务有没有变好？如果这些问题没有答案，所谓提效很可能只是在局部看上去漂亮。Headcount Reduction 不一定就是唯一能获得的收益。

企业还没落地 AI，原因常常不在模型能不能做事。更多时候，公司还没把模型能做的事，接进一段可以验收、有人负责、出了问题能接住、之后还能继续学的工作。

从回答问题到替人做事，产品能力跨了一步；从替人做事到让组织稳定地获得价值，才是更慢、更难，也更值得开发者与整个团队一起要去完成的工作。

## 参考资料

- Microsoft WorkLab, [2026 Work Trend Index: Agents, human agency, and the opportunity for every organization](https://www.microsoft.com/en-us/worklab/work-trend-index/agents-human-agency-and-the-opportunity-for-every-organization), 2026-05-05。
- Elisa Pereira, Alvin Wang Graylin, Erik Brynjolfsson, [The Enterprise AI Playbook: Lessons from 51 Successful Deployments](https://digitaleconomy.stanford.edu/app/uploads/2026/03/EnterpriseAIPlaybook_PereiraGraylinBrynjolfsson.pdf), Stanford Digital Economy Lab, 2026-04。
- Google Cloud, [State of AI-assisted Software Development 2025](https://dora.dev/research/2025/dora-report/), DORA, 2025。
- Melvin E. Conway, [How Do Committees Invent?](https://melconway.com/Home/pdf/committees.pdf), 1968。
...
- National Institute of Standards and Technology, [AI RMF: Govern](https://airc.nist.gov/airmf-resources/airmf/5-sec-core/).
...
（Microsoft，2026）: 组织准备度相关分析主要基于 AI 用户自我报告；文中将其视为组织环境的重要信号，不将相关性表述为因果结论。
（Stanford，2026）: 研究只纳入已上线、已持续使用并有可衡量价值的 51 个成功项目，因此用于理解成功条件，不用于估计企业 AI 的总体成功率。
（DORA，2025）: 软件交付组织的研究，只作为开发与工程 Agent 的侧视角，不外推为全部行业的结论。
（Conway，1968）: 本文把康威定律当作观察和设计框架，不把它写成组织结构决定系统结构的严格因果律。

...
title_en: "Parameter-Efficient Fine-Tuning (PEFT): From Adapter to LoRA"
date: 2026-03-05 13:00:00 +0800
categories: ["AI & Agents", "Training & Alignment"]
tags: ["PEFT", "LoRA", "Adapters", "Prefix Tuning", "Fine-Tuning", "Survey"]
author: Hyacehila
excerpt: 梳理参数高效微调（PEFT）领域的代表性方法——从 Adapter、Prefix-Tuning 到 LoRA、Prompt Tuning、P-Tuning v2 与 AdaLoRA，理解不同技术路线的设计思路与适用场景。
excerpt_en: "A survey of PEFT methods including Adapter, Prefix-Tuning, LoRA, Prompt Tuning, P-Tuning v2, and AdaLoRA, with their design logic and use cases."
mathjax: true
hidden: true
...
Adapter 可以算是 PEFT 领域早期的代表方法。在这个阶段，虽然大规模预训练模型已经出现，但因果语言模型尚未占据主导地位，很多下游任务仍需要通过微调适配。Adapter 正是在这个场景下，为了降低微调成本而被提出的。
...
- **模型复用**：主体模型只需要保留一份，不必为每个任务复制全量参数
...
这是对 LoRA 方法的改进。和 QLoRA、LoRA+ 等直观改法不同（分别是量化原始参数以及修改 LoRA 部分的学习率），AdaLoRA 希望引入一个 Adaptive 的 LoRA rank，减少低价值部分的 LoRA，从而提高微调效率。对于判别重要性，AdaLoRA 使用奇异值分解（SVD）来实现。
...
本文梳理了 PEFT 领域从 Adapter 到 AdaLoRA 的代表性方法演进。这些技术共同说明了一点：**预训练模型的参数空间存在大量可复用结构，低维或局部适配在许多下游任务上已经足够有效。**
...
其中，LoRA 及其变体凭借低额外推理成本和灵活性，已经成为当前最常用的 PEFT 方案之一。

展望未来，PEFT 的研究方向可能需要从单纯的高效适配，转向**适配 RL 的优化动力学**。正如 Meta 在 Three-Gate Theory 中讨论的，RL 后训练的梯度更新可能遵循与 SFT 不同的几何路径：它更倾向于修改预训练参数空间中曲率较低的非主方向子空间，而不是主成分方向。因此，传统基于低秩主权重更新的 LoRA，未必总能匹配 RL 的优化动力学。**如何设计能够保护和利用这种非主方向更新特性的参数高效方法，将是 PEFT 下一阶段值得关注的问题。**

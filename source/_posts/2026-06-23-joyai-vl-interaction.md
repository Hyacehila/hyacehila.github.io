...
如果说 JoyAI-VL-Interaction 是把 Interaction Model 放进视觉语言流里，那么 GPT-Live-1 更像是同一个问题在语音侧的版本。它不是本文逻辑的反例，而是另一个很好的补充：实时交互不只发生在摄像头和视频里，也发生在最日常的人类对话里。只是视觉侧更关心“我看到了什么，以及现在要不要介入”，语音侧更关心“我听到了什么，以及现在要不要接话”。

传统语音助手往往是级联系统：先 ASR 把语音转成文字，再交给语言模型生成答案，最后 TTS 把答案读出来。这条链路当然能工作，但它天然把对话切成几个离散阶段。GPT-Live 被 OpenAI 描述为 full-duplex speech-to-speech model，可以同时听和说，更接近人类对话里那种边听边调整、边说边感知对方反应的状态。这里的重点不是“支持语音输入输出”这么简单，而是交互协议从 turn-based chat 往连续语音流靠近。

语音里的 Interaction Model，核心行为也不一定是回答问题。它可能是在用户停顿时给一个 backchannel，可能是在对方还没说完时保持沉默，也可能是在误解即将发生时轻轻打断，或者把更复杂的任务委派给后台系统。换句话说，模型生成的不只是句子，也包括轮替、沉默、确认、打断和继续听这些对话动作。它们和 JoyAI-VL-Interaction 里的“静默、直接回应、委派”其实很像，只是模态从视觉语言换成了双向语音。

这也让 GPT-Live-1 很适合作为 Interaction Model 的另一个参照。JoyAI-VL-Interaction 判断的是“现在该不该看见并行动”，GPT-Live-1 判断的是“现在该不该说、听、等待或委派”。二者的共同点不是多模态本身，而是模型开始把交互时机纳入自身行为，而不是把它完全交给外部产品层脚本处理。

Interaction Model 是一组正在出现的模型形态，而不是某一种单一架构名。VL interaction、voice interaction、robot interaction、desktop interaction 都可能长得不一样，但它们都在把模型从离散 Chat 框里放回连续事件流。JoyAI-VL-Interaction 给了一个视觉语言侧的例子，GPT-Live-1 则提醒我们，语音本身也足够构成一个重要的实时交互世界。
...
但这个方向很值得看。它不是发明了一个和生成式模型完全不同的新结构，而是把我们从 Chat 的惯性里拽出来。视觉实时交互与语音实时交互正在从不同模态证明同一件事：模型不一定只能等用户发话；生成也不一定只生成自然语言；EOS 也不必成为交互的边界。我们以前总是在研究 VLA、自动驾驶与机器人的时候讨论实时性，但人与 Agent 之间的交互，未尝不需要这种实时性，现有的 Chat 对齐和 Agent 设计也不一定就是正确的答案。
...
这也是 JoyAI-VL-Interaction、GPT-Live-1 以及所有 Interaction Model 有趣的地方。JoyAI-VL-Interaction 是视觉侧例子，GPT-Live-1 是语音侧例子，它们都不只是给模型多接了一个输入输出通道，而是在尝试把交互时机放进模型行为里。这个看似很新的概念又回到一个很朴素的问题：如果生成式模型本来就可以沿着连续序列往前走，那为什么一定要把它关在一问一答的 Chat 框里？

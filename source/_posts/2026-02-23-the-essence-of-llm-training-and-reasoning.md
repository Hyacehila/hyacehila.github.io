...
tags: ["Reasoning", "Reinforcement Learning", "Alignment", "Training Dynamics"]
author: Hyacehila
...
在探讨大模型推理 (Reasoning)之前，需要先看清模型训练里的一个基本事实。
...
**Loss Function 是 Surrogate**。如果存在唯一正确的训练目标，就不会有这么多不同损失函数在各自领域里都能生效。

目标函数在数学上长什么样当然重要，但它不是终点。优化损失只是寻找有用参数表示的手段。如果我们不关心模型究竟学到了怎样的数据结构，而只盯着表面的 Loss，就容易陷入陷阱。最典型的例子就是 0-1 评测机制：如果我们只教模型“答对 = 1 分、回答我不知道 (I don't know) = 0 分”，这种看似合理的 Surrogate 实际上会鼓励投机，自然催生幻觉模型。

因此，与其只盯着 loss，不如从我们希望表征学到的数据结构出发，去设计符合需求的训练信号；与其只问 loss 有没有收敛，不如问这个 surrogate 是否奖励了你想要的行为（以及是否系统性惩罚了不确定性表达）。把 loss 当成 surrogate，会更自然地把注意力放回数据、约束与评测。

基于这层视角，我们可以重新审视 SFT (监督微调) 与 RL (强化学习)：**它们不应该被简单排成训练质量高低。** 两者都是在调整庞大而超高维的参数空间，只是使用的数据和 Surrogate 引导方向不同。训练方法会影响速度、稳定性和探索方式，但不一定单独决定终点。**RL 的有效性，很大程度来自 RL 数据和奖励结构；奖励本身不直接引入新知识，但可以放大特定能力。**

顺着这个代理的思路，再看推理 (Reasoning) 会更清楚。不管是 SFT 的数据，还是 RL 的奖励信号，这些 Surrogate 究竟如何从宏观和微观两个层面影响模型能力？
...
*   **对于已掌握的 ID 任务**：无论加多少强化学习，提升的主要都是首发命中率 (pass@1)。此时的 RL 更多是在利用既有能力，甚至会催生熵坍缩；它不一定扩充绝对能力边界 (即 pass@128 没有提升)。
*   **对于彻底未知的 OOD 任务**：预训练阶段如果完全没有暴露过相应的原子运算操作（哪怕只有 1% 的曝光度都会成为种子），单靠后训练的 RL 仍然无能为力，因为 RL 并没有注入新知识。
*   **边界的突破（Capability Edge）**：RL 这个强力的 Surrogate **在能力边缘最容易展现作用**。所谓能力边缘，指的是那些模型在 pass@1 上难以成功，但在多次实验的 pass@128 上偶尔能够成功的艰深任务。只有针对这些靶向任务设计训练信号，大规模强化学习才更可能实质性提升 pass@128，实现外延泛化。

这个研究处理了之前研究上的矛盾：有的研究认为 RL 只会带来熵塌缩，但是有的研究认为 RL 扩展了能力边缘（如 RLVR），原因在于训练难度谱的不同区域。对于基础模型已能解决的分布内任务，随着 pass@k 中 k 值增大，性能会趋于饱和，RL 很难再带来优势；而当 RL 瞄准基础模型表现不佳的分布外任务，且 RL 数据贴近当前模型的能力边缘时，就能观察到明显的外延泛化提升。
...
既然 RL 的收益常出现在能力边缘，那么大模型又是如何走到这个边界的？这离不开预训练（Pre-Training）、中期训练（Mid-Training）与后训练（Post-Training）三个阶段的衔接。

**预训练：无暴露则无泛化**
虽然 RL 极具边界扩展能力，但它本身并不给模型灌输全新的知识，而是在现有知识范围内对能力边缘进行外推。研究表明，**若预训练阶段针对某个概念或长尾语境完全是 0 暴露，无论后续投入多少探索，RL 都将无能为力，无法诱导向该未知语境的迁移**。然而，只要预训练里哪怕只有极少量相关暴露（如最基础的原子运算片段），就能为后续 RL 提供至关重要的能力种子。依靠这颗种子，基础模型便获得了必要的初步推理原语，后续 RL 才能将其精准放大，实现稳健的跨语境泛化。因此，预训练的作用不在于教会模型严密推演，而在于尽可能广地铺设认知与基础概念的覆盖。
...
RLVR 则进一步验证了 RL 和 SFT 在 surrogate 视角上的同质性。它给出的口语化直觉很朴素：如果预训练已经让模型具备基本的 knowledge / logic prior，那么**正确 CoT 比错误 CoT 更容易导向正确答案**。于是即便 RLVR 只看最终答案，梯度在期望上也会更常强化那些更干净的推理路径。换句话说，answer-only RLVR 不只是把已有答案采样得更快，它还会逐步把 rollout 里的 CoT 洗得更像可学习的数据。
...
宏观部分讨论的是数据与任务边界，微观部分则分两步看：先看 CoT 训练如何把推理步骤写进模型内部，再看 RL 如何通过熵管理、token 分叉与参数几何改变模型行为。
...
这件事的意义不只在于“可解释”。与不带 CoT 的训练相比，中间结果能够在更浅层被读出，说明更深层还保留着处理后续步骤的容量。因此，论文里那句很有传播力的总结——CoT 更像是在教模型 *how to think*，而不只是 *what to think*——放在这里是成立的，但语境需要收紧：它说的是显式步骤训练下的组合式 reasoning，而不是所有推理任务的无条件定律。
...
这篇论文从理论和实证两端都说明，策略熵的变化与 advantage 驱动的 logit 更新密切相关；经验上，作者还观察到一个近似关系：$R = -a \exp(H) + b$。它指向一个很现实的结论：不少 RL 性能提升，其实是在消耗探索熵。如果缺少额外干预——例如文中讨论的 KL-Cov 选择性约束，或更传统的熵正则化——模型就很容易沉迷于局部高分捷径，最终出现 reward hacking 式的短视行为。
...
也正因为如此，对所有 token 一刀切地回传梯度很可能是资源错配。这篇论文的核心发现更进一步：如果把策略梯度主要集中在这少数高熵 token 上，效果往往可以逼近甚至超过全量 token 更新。它支持的是“RLVR 的有效性主要来自关键分叉 token”，而不是 CoT training 本身的理论机制。

### Three-Gate Theory：RLVR 走向稀疏更新

前一节谈的是 token-level / policy-level 的熵与分叉；这一节转向另一类问题：参数几何如何把 RL 更新筛成稀疏、局部且偏向非主方向的改动。

当我们审视高成本、高收益的特定强化学习（如 Meta AI 论文重点探讨的基于程序验证奖励的 RLVR）时，会发现一个反直觉现象：**这种 RL 方法引发的参数变化比简单 SFT 更稀疏，并且集中在特定区域。问题是，这种只针对少数神经元矩阵的更新，为什么仍然能明显改变推理表现？**

Meta 的研究论文 *"The Path Not Taken: RLVR Provably Learns Off the Principals"*，用 **三门理论 (Three-Gate Theory)** 给出了一个解释框架。

1.  **Gate I（策略约束门，KL Anchor）**：在每一轮 on-policy 更新的端口，KL 锚点强行对更新步伐做了限制。它确保每一次尝试都在预训练既有模型的熟悉辐射带内。
2.  **Gate II（模型几何门，Model Geometry）**：大语言模型在预训练时，参数矩阵已经形成一个巨大的“高维 Landscape”。在这个空间里，存在一些主方向（即曲率最大、模型原有知识权重最稠密的通道）。当 RLVR 的梯度开始修改参数时，模型的底层几何结构会让这些更新更容易避开主骨干方向（Off the principal directions），转向曲率较低、不明显破坏既定光谱特征的子空间边缘。这解释了为什么特定模型会偏好某些微调路径，也说明这种偏好和预训练形成的地貌有关。
...
这种动力机制提醒我们：在走向后训练时代时，直接照搬 SFT 时代的 PEFT (参数高效微调，比如只依赖于低秩分解的标准 LoRA) 可能不够稳。传统的低秩主权重更新路径，可能与这种偏向非主曲率方向、高熵边缘、微小调整的需求不完全匹配。新的参数高效方法需要考虑如何保护和利用这部分流形。

## 结尾：不同的代理，不同的数据，不同的目标

所以，大模型推理不必被理解成魔法。预训练、中期训练、SFT 与 RLVR 使用不同阶段的数据和损失代理，持续塑形模型能力。**模型能力改进，很大程度上依赖于设计更合适的 Surrogate，让目标表征更容易被学习。**

但大模型的目标不该只有推理。无论是工具使用、角色扮演、记忆模块还是规划能力，只要有更合适的 surrogate 坐标，参数空间里的可用能力就可能被逐步调出来，而不一定非要等待某个全新算法。算法很多时候决定的，是我们发现和稳定这些能力的速度。

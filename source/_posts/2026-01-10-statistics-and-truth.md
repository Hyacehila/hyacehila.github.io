...
这不是一本专业书籍，它回顾了统计学的历史以及发展中产生的诸多问题。全书借助例子讲述统计思想，说明统计学如何成为一门科学，而不是事实的堆砌。
...
直到20世纪初叶，人们才开始尝试将不确定性数量化，以回答这些问题。我们还不能说这个努力已经充分成功，但已取得的成果已经影响了许多人类活动领域：它带来新的研究设想，促进自然科学知识的发展，也改变了我们的思考方法。

随机数列显示了最大限度的不确定性（或称为混沌或者熵），它也是我们研究不确定性的最常用方法。随机数列是不遵循任何特殊模式的数列，可以使用各种方法来产生，包括一些观察自然得到的随机数列以及一些人工的产生方法。

有许多开发利用随机性的方法，使我们能为一些棘手的问题找到突破口，处理难以求得精确解答的问题，产生新的信息，并可能帮助发展新的思想。比如 **Monte Carlo 方法**、**随机抽样调查**以及**实验设计方法**。随机甚至可以用于解决一些博弈与决策理论中的问题。
...
然后要求被提问者投掷一个硬币，出现正面时正确回答 S，出现反面时正确回答 T。这时提问者并不知道被问者回答的是哪一个问题，这个信息是保密的。从这些得到的答案可做如下估计，推算出吸大麻的人所占的比例。
...
但人们逐渐发现这并不可能。由于存在测量误差，人类很难准确了解系统的初始状态。在这种情形下，初始状态的微小差别可能导致对系统后续状态预报的极大差异。随机性进入自然科学领域时，同时出现了三个发展：
...
统计学思想远古即存，但作为一门学科却历史很短。统计学的起源可以追溯到人类的原始时期，但直到近代才逐渐成为实际应用中的重要学科。所有这些发展，引出了一连串关于统计学是什么的问题。
...
前面已提到，统计学词根的意义是指对数据的收集和整理，并使其用于公共政策的制定。但统计学不仅仅是数据本身，它还包括对含有不确定性的数据进行研究与分析的方法。
...
由这个基本方程可以导出风险管理的一个有效方法，也把人类从神谕和算命先生中解放出来。它把未来置于现时可做出明智决策的框架之中：
...
至此，从数据中获取信息并做出推断的新学科产生了，统计学这个术语的范围也从数据自身扩展到解释数据。偶然性不再只是一件值得担心的事情或者一种无知的表现。相反，它成为表达我们拥有知识的一种合乎逻辑的方法。
...
统计分析的形式随时代推移而变化，但“从数据中提取一切信息”或者“归纳和揭示”作为统计分析的目的却一直没有改变。统计分析的方法多年来不断更迭，下面对数据分析的发展历史作一概述。
...
如果在制定计划时引进统计学的思想，引进可以用来有效地分析数据和评价反馈和控制结果的统计方法，可以提高人类活动的价值。

可以说：如果有什么问题要解决，应求助于统计学，而不只是某个专家委员会。比起收集少数专家的智慧，统计学和统计分析能为解决问题带来更多线索。

...
tags: ["Cost Engineering", "Backend Engineering"]
author: Hyacehila
...
### Mask 工具而非增删，保持 append-only

前面讲的是"内容怎么排"，还有一个同样会悄悄毁掉缓存命中率的维度："上下文怎么变"。Manus 在 [Context Engineering for AI Agents](https://manus.im/blog/Context-Engineering-for-AI-Agents-Lessons-from-Building-Manus) 里给了两条工程纪律，都是在保护这个稳定前缀。

第一条是 **mask，而不是动态增删工具**。很多 Agent 喜欢按当前阶段动态地往上下文里加减工具定义，这是双重灾难：工具定义通常排在上下文很靠前的位置（属于稳定前缀的一部分），一改就让它后面的 KV-Cache 大面积失效，每轮都得重新 prefill；而且上下文里若引用了一个已经被移除的工具，模型会困惑甚至幻觉出对它的调用。更好的做法是让工具定义**始终不变**，用一个状态机在解码时 **mask 掉不该用的工具的 logits**——工具还在上下文里，只是当前这一步"不可选"。配合一致的工具名前缀（如 `browser_`、`shell_`），还可以按组约束动作空间，而完全不动前缀。

第二条是**上下文 append-only**。不要回头去修改前面已经发生的动作和观察——任何对历史的原地改写都会让那一点之后的前缀全部失效。与之配套的是**序列化要确定**：同样的内容每次要序列化成同样的字节，比如 JSON 的 key 顺序必须稳定，否则一个看不见的字段重排就会悄悄破坏缓存命中。换句话说，前缀稳定不只要求"内容相同"，还要求"表示相同"。

这两条和前面"稳定的内容靠前、动态的内容靠后"是同一个原则的延伸：**把整个上下文当成一条只增不改的稳定前缀来经营，而不是每轮重新拼装的临时缓冲区。**

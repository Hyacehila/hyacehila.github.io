...
tags: ["Scaling Laws", "Pre-Training", "Chinchilla", "Compute Scaling", "Model Mechanics", "Paper Notes"]
author: Hyacehila
...
在一定范围内，成倍增加算力、数据或参数，模型的 Loss（损失函数值）通常会以相对稳定的速度下降。这让炼丹不再完全靠直觉，而是多了一套未必精确但足够有用的工程估算工具（P.s. 新时代材料大炼丹，对实验人员除了头发以外的地方都极其友好）。

Neural Scaling Laws 最基本的公式形式通常为：
$$ L(x) \propto x^{-\alpha} $$
其中 $L$ 是测试集上的 Loss，$x$ 是规模变量（如参数量 $N$、数据量 $D$ 或 算力 $C$），$\alpha$ 是缩放指数。
**主要结论：** 在实验覆盖的范围内，模型性能主要受规模（Scale）影响，而与具体的模型架构（如层数、宽度比例）关系较弱（只要架构不是太离谱）。
...
这常被类比为大模型时代的"摩尔定律"。更准确地说，它用实验拟合显示 Transformer 模型的性能与 $N$（参数量）、$D$（数据集大小）、$C$（计算量）之间存在稳定的幂律关系。
...
在总算力 $C \approx 6ND$ 固定的约束下，利用拉格朗日乘数法求极值，DeepMind 发现 $\alpha \approx 0.5, \beta \approx 0.5$。 由于 $\alpha \approx \beta$，$N$ 和 $D$ 对 Loss 的贡献近似**对称**。 在固定总算力下最优策略是 **$N$ 和 $D$ 应该等比例增长**（$N \propto C^{0.5}, D \propto C^{0.5}$）。这就是著名的"Chinchilla Scaling Laws"。
...
这使得 MoE 有机会在 **Pareto Frontier（帕累托前沿）** 上优于 Dense 模型：**在同等推理算力下，MoE 往往能调动更大的总参数容量。**

### 结语
神经缩放定律把大模型训练中的一部分经验规律写成了可估算的工程关系。Kaplan 强调参数规模，Chinchilla 把重点修正到算力与数据的配比；能力"涌现"、Data Wall、数据质量和 MoE 架构，都是在这条缩放路线继续推进时暴露出来的问题。随着 Training Scaling 的边际收益变得更贵，后续路线一方面会继续寻找更高质量的数据和合成数据，另一方面也会把更多注意力放到 Inference Time Scaling（推理时扩张）上。

## Bias-Variance No Trade-off：Neural Scaling Laws and Statistical Learning Theory

神经缩放定律（Neural Scaling Laws）之所以重要，不只是因为它能指导工程估算，也因为它和传统统计学习理论（Statistical Learning Theory, SLT）里的直觉存在明显张力。

传统的 SLT 告诉我们"模型太大会过拟合"，做一个更好的模型需要控制模型的复杂度，而 Scaling Laws 告诉我们"越大越好"。这种张力来自**偏差-方差权衡（Bias-Variance Trade-off）在深度学习时代的失效与重构**。

### 经典回顾：偏差与方差的传统视角 (Traditional View)

在深入讨论冲突之前，我们需要先回顾一下经典统计学习理论是如何看待泛化误差的。

**偏差-方差分解 (Bias-Variance Decomposition)** 是解释学习算法泛化性能的常用工具。对于一个学习算法，其在测试集上的期望泛化误差 $E(f;D)$ 可以分解为三项之和：

$$
E(f;D) = \text{Bias}^2(\boldsymbol{x}) + \text{Variance}(\boldsymbol{x}) + \text{Noise}
$$

*   **偏差 (Bias)**：衡量了模型的拟合能力。偏差高意味着模型太简单，无法捕捉数据的真实模式（**欠拟合**）。
*   **方差 (Variance)**：度量了模型对训练数据变动的敏感性。方差高意味着模型太复杂，记住了训练数据中的噪声（**过拟合**）。
*   **噪声 (Noise)**：数据本身的不可约误差，是泛化的下界。

**偏差-方差窘境 (Bias-Variance Dilemma)**：
这是传统理论中最令人头疼的权衡。
*   **训练不足时**：Bias 主导，Variance 低。学习器的拟合能力不够强，训练数据的扰动不足以使学习器产生显著变化。
*   **训练过度时**：Bias 低，Variance 主导。学习器对训练数据拟合得太好，以至于捕捉到了数据中的噪声。

这导致了著名的 **U 型学习曲线**：随着模型复杂度的增加，总误差先降后升。因此，训练需要在一个中间点停下来，寻找 Bias 和 Variance 都不太高的“甜蜜点”（Trade-off）。
...
然而，2017 年 Zhang et al. 在 *Understanding deep learning requires rethinking generalization* 中提出了一个很有冲击力的现象。他们发现：**即使把训练集的标签 $y$ 全部随机打乱（纯噪声），深度网络依然能达到 $\hat{R}(f)=0$**。这说明模型容量足以记住纯噪声，但在真实数据上仍可能泛化；传统基于"一致收敛"的上界，很难解释深度学习里的实际表现。

### 逃离诅咒：良性过拟合 (Benign Overfitting)

为了解释 Scaling Law（即 $N \to \infty$ 时 Loss 单调下降），学界提出了 **良性过拟合** 理论（Bartlett et al., 2020）。这一理论试图重新理解 **方差（Variance）** 在高维空间的行为。
...
- **Variance 消失（而非爆炸）：** 这是最重要的部分。Bartlett 证明，只要数据协方差矩阵满足特定的光谱衰减，**噪声能量会被"涂抹"分散到无数个多余的维度上**。
...
这两部分内容把神经缩放定律（Scaling Laws）从经验公式带到了工程资源分配和历史演变上。
...
在实际的大模型训练中，我们关心的不仅仅是"参数 $N$"或"数据 $D$"本身，而是 **"我有 1 亿美元的算力预算（FLOPs），我该怎么花？"**  这时 Scaling Law 就变成了一个资源分配优化问题。
...
偏离这个最优组合会有相应代价。**过大模型（Over-sized）** 指的是如果你用 huge 的模型但数据很少（如 Kaplan 早期建议），你在浪费算力做矩阵乘法，却没有给模型足够的信息（Undertrained）。相反，**过小模型（Under-sized）** 指的是如果你用 tiny 的模型跑了海量数据（如 LLaMA 之前的不少小模型），模型容量已满，继续读数据的收益很低。
...
**Chinchilla 定律可以写成这个优化方程：**
$$ \min_{N,D} L(N,D) = \frac{A}{N^\alpha} + \frac{B}{D^\beta} \quad \text{s.t.} \quad C = 6ND $$
解得最优分配是 $N \propto C^{0.5}, D \propto C^{0.5}$。这意味着在这个设定下，算力预算会直接约束模型该做多大、数据该喂多少。


### 架构无关性 (Universality)：为什么最后赢的是 Transformer？

#### 架构无关性 (Universality)
Scaling Law 一个很有用的观察是：**幂律趋势不只出现在某一种架构上。**

OpenAI 和 DeepMind 的研究表明，不只是 Transformer，包括：
*   **LSTM / RNN**（循环神经网络）
*   **CNN**（卷积神经网络）
*   **Linear Transformers**
*   甚至一些纯全连接网络

它们的 Loss 随规模增长都可以观察到类似 $L \propto C^{-\alpha}$ 的幂律形式。这意味着，LSTM 这类架构在理论上也能从更多算力和参数中受益。**缩放收益并不是 Transformer 独占的。**

既然都遵循定律，为什么 LSTM 死了？ 这涉及到一个关键区分：**理论性能上限 vs. 工程实现效率**。

虽然它们都遵循幂律公式 $L = a C^{-k}$，但其中的系数 $a$（截距）和 $k$（下降斜率）有所不同，更要看**实际训练的可行性**。

#### 效率即一切：串行瓶颈与梯度流

**串行 vs. 并行 (The Wall-Clock Time Wall)**

这是 Transformer 胜出的重要因素。**LSTM/RNN** 的数学形式 $h_t = f(h_{t-1}, x_t)$ 决定了它必须串行计算：要计算第 1000 个 token 的状态，必须先算完前 999 个。这种 $O(T)$ 的时间依赖很难并行，导致即使它理论上能 Scaling，实际训练到同等规模也会非常慢。

相反，**Transformer** 的 Attention 机制 $Softmax(QK^T)V$ 允许一次性并行计算所有 token 之间的关系。虽然复杂度是 $O(T^2)$，但它更容易吃满大规模 GPU 集群的矩阵计算能力，训练效率在当前硬件上明显占优。

**梯度流与长距离依赖**

除了计算效率，梯度流也很重要。在 **LSTM** 中，梯度需要通过时间步反向传播（BPTT），在处理极长序列（如一本书）时容易丢失早期信息，导致 Scaling 曲线比 Transformer 扁平。而在 **Transformer** 中，任意两个 token 之间的距离都是 1（通过 Attention 连接），更通畅的梯度流使其在 Scaling 时能更有效地利用数据。
...
这也是为什么现在的 AI 实验室更像是**系统工程公司**，而不是传统的算法实验室。竞争力从"设计精巧的网络结构"转向"构建能稳定运行万卡集群的基础设施"。

## 反面教材：GBDT 的"硬切割"与失效的 Scaling

这是一个很有实战意义的问题。Scaling Laws 在经典的梯度提升树（GBDT，如 XGBoost、LightGBM）面前并不直接成立，这反而能帮助我们厘清 Scaling 依赖什么条件。
...
相比之下，LLM 处于"双重下降"的现代区间。在数据量足够（符合 Chinchilla 比例）的前提下，增加参数量通常能带来 Loss 的持续下降。这种"堆参数"的红利在深度学习里更明显。

### 分段常数 vs 连续流形：归纳偏置的战争

为什么？关键在于二者数学上的 **归纳偏置（Inductive Bias）**，也就是模型预设的"世界观"不同。

#### GBDT：空间的硬切割 (Space Partitioning)
树模型可以理解为将高维空间切割成许多互不相关的**超立方体（Hyper-rectangles）**。
...
平滑性会直接影响 Scaling Law 能否生效。在高维空间中，数据极度稀疏。模型必须根据训练点去猜测（插值）空白区域的值。如果模型是"跳跃"的（如 GBDT），空白区域的预测就容易不准确（高方差）。如果模型是"平滑"的（如 DNN），它就能利用已有的数据点进行平滑插值，从而在空白区域保持较低误差。
...
**Scaling Law 很依赖平滑性（Smoothness）。** GBDT 缺乏全局平滑性，过参数化更容易导致空间碎片化；而 DNN 具备全局流形结构，更容易把过参数化转化为对真实函数的**高精度平滑插值**。
...
答案是：**是的，这是一种 Scaling，但它更接近"非参数化 Scaling"，与深度学习的"参数化 Scaling"在动力学上不同。**

我们可以尝试建立这种联系，但也要看到它们在**数学机理**上的差异。以下是基于之前讨论的进一步发散：

### 重新审视：树模型真的只有 U 型曲线吗？（连接 Double Descent）

前面提到树模型通常遵循 U 型曲线，但这只是一个"有条件的真理"。最新的研究（如 Mikhail Belkin 团队的工作）表明，**在特定条件下，树模型也能展现出 Scaling Law 式的单调下降或"双重下降"现象。**

#### 随机森林 (Random Forest) 的"良性过拟合"
随机森林是更接近 Scaling Law 直觉的传统方法。
...
**与 Deep Learning 的联系：** 这很像深度学习中的**宽度扩展（Width Scaling）**。超宽的神经网络也可以从集成（Ensemble）的角度理解为许多子网络共同作用。
...
**网络的扩展：** 当网络参数足够多，在合适优化和正则条件下，它更可能学到穿过样本点的**相对平滑流形**。

**测试时：** 对于新样本，它是在流形上游走。

**结论：** Scaling Law 真正需要解释的，不只是"变大"，还包括"变大后为什么仍能保持相对平滑的预测结构"。树模型变大后（如果不强行加正则），空间会更容易变得**破碎**。

### 如果让树模型适配 Scaling Law？

如果希望树模型拥有像 Transformer 那样的 Scaling 能力，需要怎么做？这指向了当前 AI 研究的一些融合方向。
...
*   **结果：** 这种"神经树"就可以使用 SGD 训练，也更接近神经网络的缩放条件。
*   **启示：** 这说明，Scaling Law 相关收益可能不只取决于架构名字（树 vs 网），也取决于**可微分性（Differentiability）**带来的**梯度优化**和**连续流形表示**。

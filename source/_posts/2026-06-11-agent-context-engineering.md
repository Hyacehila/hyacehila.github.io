...
单次请求里的任务、示例和输出契约怎样组织，属于更基础的 Prompt 与上下文学习问题，可以先参考[《提示工程与上下文学习》](/blog/2024/09/20/prompt-engineering-and-in-context-learning/)。本文从多轮、长任务和运行时调度继续往下走。

## 正文开始之前

正文会从存储结构讲到运行时手法。先交代三件事：这篇在整个系列里处在哪一层，上下文工程在这里具体指什么，以及为什么上下文应该被当成一种需要调度的有限资源。
...
本文会停在工程实现，把常见手法讲清楚，但不逐个产品横评。读到某个机制时，如果你想知道具体系统怎么实现、代码每一行怎么写，答案在 teardown 篇里。
...
先看存储。但别一上来就问用哪个数据库，先问 Agent 在运行时读写的到底是什么对象。Memory 全景图已经解释过为什么不同记忆适合不同载体：关系密集的语义知识适合图，操作性经验和规范适合文件，事实记录适合关系数据库。这里不重复选型理由，而是讨论工程上怎么实现，以及这些载体如何一起服务当前窗口的调度。
...
有了存储结构，下一步就是把外部信息接回当前推理现场。前一章讲的是外部记忆怎么存成可治理对象，又怎么投影到关系库、全文索引、向量索引、图和文件系统里。Read Path 关心的是另一侧：任务发生时，runtime 怎么从这些投影里挑出少量证据，让它们带着来源、权限和版本信息进入 Working Memory，去指导真实决策。这里要把这条工程链路讲清楚。
...
Context Engineering 的重点不是把窗口做长，也不是堆一个向量库。更实用的标准是：系统能不能说明哪些信息进入当前上下文，哪些被压缩或换出，哪些被隔离在子 agent 里，哪些被写入长期记忆，以及这些决定为什么发生。窗口会继续有限，注意力也仍然是预算。只要这个前提不变，把上下文当成有限资源来调度，就是 Agent 工程的一项基础能力。

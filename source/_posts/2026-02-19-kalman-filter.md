...
title_en: "Kalman Filtering: From Apollo Landings to Global Weather Forecasting"
date: 2026-02-19 20:00:00 +0800
categories: ["Data Science & Statistics", "Time Series & Spatial Data"]
tags: ["Kalman Filter", "State Space Models", "Time Series", "Filtering"]
author: Hyacehila
hidden: true
...
### 为什么需要卡尔曼滤波
状态估计（State Estimation） 是现代机器人、自动驾驶和航空航天技术中的基础问题。工程实践需要在多个信息来源之间取得平衡，得到可靠估计值。Kalman Filter 正是解决这个问题的经典算法，也展示了统计学如何用确定的计算流程处理不确定性。

我们希望获得精准的定位，假设有两个信息来源，但它们都有误差：
1. 你的车速表：显示 100 km/h，但实际可能在 90-110 km/h 之间。
2. 你的 GPS：显示位置，但信号漂移严重，可能在 50 米范围内乱晃。

卡尔曼滤波的基本思路是：既然两者都有误差，就通过加权平均得到更可靠的估计。

如果传感器很准（观测误差小），就多相信传感器；如果车身很稳（模型误差小），就多相信物理公式。这个“相信多少”的比例系数，就是**卡尔曼增益（Kalman Gain, $K$）**。
...
首先，需要用数学语言描述这个系统。假设系统是**线性**（这是卡尔曼滤波的局限性，后面会继续处理这个问题）的：
...
卡尔曼滤波凭借递归形式和统计学上的最优性，长期是状态估计领域的标准工具。虽然这里介绍的是最基础的线性卡尔曼滤波，但它解决的不只是“怎么算得准”，还提供了一种工程思路：**不要轻信单一信源，要用概率的眼光看世界。**
...
**EnKF 为什么有效**
1.  **避开了雅可比矩阵：** 不需要求导，直接放进模型运行，更适合处理非线性。

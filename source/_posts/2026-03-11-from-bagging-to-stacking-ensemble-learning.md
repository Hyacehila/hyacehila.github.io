...
这篇文章中的问题也可以和[机器学习导论：监督学习与贝叶斯方法](/blog/2024/03/28/machine-learning-introduction-supervised-learning/)、[机器学习进阶：无监督学习与半监督学习](/blog/2024/04/06/advanced-machine-learning-unsupervised-learning/)放在一起阅读，以比较相近的概念如何在不同语境中展开。
...
用两个关键词概括集成学习，我会选 **多样性** 和 **偏差-方差**。

第一，多样性（diversity）是集成有效的前提。多个模型如果犯的是同一类错误，把它们平均起来也不会神奇地变好；有价值的是：不同模型在不同样本、不同特征子空间、不同归纳偏好上各自有强弱，于是它们的错误能够相互抵消。

第二，不少集成方法都可以放回偏差-方差（bias-variance）框架来理解。直觉上看：
...
- **Bagging**：并行训练多个模型，再做平均或投票。
- **Boosting**：串行训练多个模型，每一轮都针对前一轮的不足继续学习。
- **Stacking**：先训练一批基模型，再训练一个元模型学习如何组合它们。

所以“集成学习”不只是一个工具箱标签，也是一套理解模型关系的方法。树模型文章里看到的 Random Forest 与 GBDT，是这套框架里的两个典型分支。

这一节先不急着背算法名字，而是看 ensemble **如何制造多样性**，以及它**试图解决偏差还是方差的问题**。这两个坐标定住后，后面的方法就容易放回同一张地图里。

## 从最简单的融合开始：Averaging 与 Voting

很多时候，集成学习从一个很朴素的问题开始：**如果我已经有了多个模型，能不能直接把它们合起来？**
...
背后的直觉很简单：单个模型会波动，但多个模型的均值往往更稳定。实际工作里，除了均值，也有人会在异常值较多时使用中位数聚合，因为它对极端预测更不敏感。
...
Averaging 和 Voting 看起来“很基础”，但后面不少框架都包含一个融合步骤。区别只是：有的方法只做最后一步融合，有的方法则连“子模型是怎么生成的”也一并设计好。

Averaging / Voting 是 ensemble 的最小原型。它们说明，哪怕不改变基模型本身，**只要组合方式得当，预测就可能更稳定**。但如果想持续拉开性能差距，就需要继续思考：这些模型是怎么被制造出来的？

## Bagging：用重采样制造稳定性

Bagging（Bootstrap Aggregating）的思想是：**既然多样性很重要，那就主动制造多份不同的训练数据。**
...
Bootstrap 还顺带给了 Bagging 一个实用的副产品：包外样本（out-of-bag, OOB）。一轮 bootstrap 抽样之后，平均来看大约会有 36.8% 的样本没有被抽中，它们就可以拿来做一个近似的验证集，用于粗略评估泛化表现，而不一定非要额外切一块 hold-out 数据。
...
这也是 Random Forest 往往很适合作为 baseline 的原因。它的训练可以并行，调参路径相对清晰，对异常值和特征缩放也通常不太敏感。在许多结构化表格任务里，如果你还没想好要不要直接上 Boosting，Random Forest 往往是很好的第一步。

当然，Bagging 也有它的边界。它更擅长“把不稳定的模型平均稳定”，但如果单个模型本身存在较强系统性偏差，仅靠并行平均通常很难不断突破这个上限。这也正是 Boosting 要接手的地方。

Bagging 的关键词是 **并行、重采样、降方差**。Random Forest 则是在 Bagging 的基础上进一步增加特征层面的随机性，因此成了决策树集成里最经典、也最耐用的一条路线。

## Boosting：把前一轮没学好的部分继续学下去

Bagging 关心“怎么让模型更稳”，Boosting 更关心：**怎么让模型一轮一轮地变得更强。**

它让基学习器按顺序串起来。后一轮学习器的任务，不是从零开始重新学一遍，而是重点处理前一轮还没学好的部分。
...
这里重点不在公式本身，而在两层“加权”思想：

- 对于被前一轮错分的样本，提高下一轮训练时的关注度
- 对于表现更好的弱学习器，给它更高的最终组合权重

因此，AdaBoost 的重点不是简单做平均，而是**让学习过程带着记忆**：前面做错的地方，后面重点补。

不过，AdaBoost 主要通过“样本重赋权”推进学习。这个思路很优美，但当损失函数更一般、任务更复杂时，我们会希望有一种更统一的观点来描述“下一轮到底该学什么”。这就通向了提升树（boosting tree）和 GBDT。

### 提升树与 GBDT：从纠错走向函数逼近

在提升树里，我们把最终模型看成多棵树的加法模型：

$$
F_M(x) = \sum_{m=1}^{M} T_m(x)
$$

这时每一棵新树的任务，都不是独立完成一次预测，而是在当前模型的基础上再补一块。问题变成了：**这一轮到底该补什么？**

GBDT（Gradient Boosting Decision Tree）给出的答案是：去拟合当前损失函数的负梯度。在第 $m$ 轮，我们关心的是
...
这一步很关键，因为它把 Boosting 从“样本权重如何变化”推进到“函数如何一步步逼近目标”的视角。也因此，GBDT 才成为后续 XGBoost、LightGBM、CatBoost 等工程实现的共同母体。上一篇树模型文章里讨论的那些框架，虽然在正则化、分裂策略、类别特征处理和工程优化上各有差异，但它们都仍然站在这条 Boosting 主线上。

理解 GBDT 的价值，并不只是为了会写一个老算法名词，而是为了看清：为什么表格建模里那些最常用的强模型，不少都来自“逐步拟合残差/梯度”这条路线。

Boosting 的关键词是 **串行、纠错、加法模型**。AdaBoost 用样本重赋权表达这种思想，GBDT 则进一步把它推广成“每一轮去拟合损失函数的负梯度”。理解到这里，XGBoost / LightGBM / CatBoost 就会清楚很多。
...
这里 OOF 是 Stacking 最关键的细节。如果直接用“基模型在训练集上的预测结果”去训练元模型，元模型看到的就是被严重高估的表现，很容易把数据泄漏当成真实能力，从而在验证集或测试集上明显翻车。

### 为什么元模型通常不宜太复杂

不少人一想到 Stacking，就会本能地觉得“既然都堆到第二层了，元模型当然越强越好”。但实战里常见的经验恰恰相反：**元模型通常不需要特别复杂。**

原因很简单。元模型面对的输入，不再是原始特征，而是已经被基模型压缩过的一组预测信号。这些信号维度通常不高，而且容易共线。如果这时再上一个过于复杂的元模型，学到的往往不是稳健的组合规律，而是验证集噪声。
...
Stacking 的关键词是 **元特征、OOF、二层学习**。它和 Bagging / Boosting 的区别，不在于“模型数量更多”，而在于它把“如何组合”本身也变成了一个可学习的问题。
...
所以我更倾向于把 DCS / DES 看成一种扩展视角：它提醒我们，ensemble 不一定只能是“一个全局规则”；但在多数工程场景里，固定规则的 Bagging / Boosting / Stacking 仍然更常见、更易管理。

DCS / DES 的贡献，在于把问题从“全局怎么加权”推进到“局部应该信谁”。它们很有启发性，但通常不是表格建模工作流里的第一优先级。
...
它的重点并不是“组合多个模型”，而是允许系统在不确定时先不要做高置信度判断，或者把样本放进一个“暂不决策”的类别。这样做常见于高风险分类任务，因为与其强行给出一个可能错误的标签，不如显式承认“不够确定”。
...
- **多样性不是乱堆模型。** 有价值的是归纳偏好、训练数据、特征视角或损失函数层面的差异。
- **Stacking 最怕数据泄漏。** 没有 OOF 的“堆叠”，常常只是把训练集过拟合得更漂亮。
...
回到文章一开始的问题：为什么 Random Forest 和 GBDT 都是“树的集成”，但行为差异这么大？答案已经很清楚。**前者是在做并行平均，后者是在做串行纠错。** 一旦把它们放回 Bagging 与 Boosting 这两条主线，树模型文章里那些具体框架的来龙去脉也就更容易看懂。

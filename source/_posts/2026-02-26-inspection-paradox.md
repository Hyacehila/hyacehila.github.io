...
title_en: "Why Data Keeps Fooling You: The Counterintuitive Inspection Paradox"
...
检查悖论的逻辑很直接：**当你去观察或“检查”某个事物时，那些“更大”或“更长”的样本，因为占据了更多的空间或时间，拥有更高的概率被你抽中。** 于是，直觉中的概率会低估真实概率，真实概率本身也会显得反直觉。
...
在严谨的数学框架下，检查悖论可以表述为一个 **长度偏差采样（Length-Biased Sampling）** 问题。
...
因为 $\int_0^\infty x^2 \cdot f(x) dx$ 就是 $E[X^2]$，代入方差公式 $E[X^2] = \mu^2 + \sigma^2$，可以得到检查悖论的推导：

$$E[Y] = \frac{\mu^2 + \sigma^2}{\mu} = \mu + \frac{\sigma^2}{\mu}$$

**结论明显违背直觉：**

因为方差 $\sigma^2 \ge 0$，所以 $E[Y] \ge \mu$ 永远成立。**只要系统存在波动，你观测到的平均值就必然大于真实的平均值。** 波动的方差越大，你感受到的悖论偏差就越离谱。

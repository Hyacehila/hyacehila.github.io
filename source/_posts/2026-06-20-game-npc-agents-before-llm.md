...
最近聊 NPC 智能时，很多人的第一反应已经变成了 LLM 对话、角色扮演、长期记忆、会自己接任务的数字伙伴。这符合最近的流行趋势，很多研究游戏 NPC 的人现在也在做这些。但把视角稍微往前挪一点，游戏行业很早就在做一种经典的 Agent 系统：NPC 观察世界，维护内部状态，选择下一步行动，再通过移动、攻击、交易、对话、逃跑或协作去改变世界。LLM 才出来几年，游戏 AI 至少已经做了十几年。

这篇文章中的问题也可以和[游戏行业如何引入 AI Agent](/blog/2026/05/05/ai-agent-game-industry-pipeline/)、[当我们谈论 AI Native Game 的时候，我们在谈论什么](/blog/2026/07/12/ai-native-game/)放在一起阅读，以比较相近的概念如何在不同语境中展开。

这套传统游戏 AI 通常不叫 Agent。它可能叫敌人 AI、战斗 AI、队友 AI、怪物行为、Boss 逻辑、NPC Brain，或者干脆叫行为树。但从系统结构上看，它很符合 Agent 的基本定义：被放在一个环境里，有可观察的信息，有目标或偏好，有可执行动作，也会因为环境反馈而改变下一步行为。

这篇文章不讨论“让 NPC 接入大模型聊天”这一类新问题，而是从生成式模型出现之前的游戏 AI 讲起。把 NPC 看成一种传统 Agent，看看它怎样被拆成感知、状态、决策、行为和动作；再用小白能读懂的方式解释脚本、有限状态机、行为树、效用 AI、GOAP 和 HTN。最后回到今天的 LLM Agent，看看新旧两种 Agent 像在哪里，又不一样在哪里。回顾这些东西，是为了更好理解现在该做什么。
...
后面就按这条线看。每种方法先讲它解决什么问题，再用一个虚构游戏场景说明它怎么工作。
...
LLM 可以让 NPC 更会说、更会解释、更能连接玩家意图，但可信的游戏 Agent 仍然依赖感知边界、状态管理、动作约束、调试工具和设计目标。难的不是让角色说出一段漂亮台词，而是让它在游戏规则里持续、稳定、可控地行动。只要它能观察环境、维护状态、选择行动、接收反馈，它就已经站在 Agent 的范畴里。语义层是否接入语言模型，反而是建立在传统游戏 AI 地基之上的下一层选择。

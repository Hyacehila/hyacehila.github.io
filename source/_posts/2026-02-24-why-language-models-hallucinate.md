...
tags: ["Hallucination", "Evaluation", "Training Dynamics", "Model Behavior", "Paper Notes"]
author: Hyacehila
excerpt: 基于 OpenAI 团队论文《Why Language Models Hallucinate》：幻觉并非单纯源于数据噪声或模型缺陷，也和现代训练范式与二元评估机制带来的统计压力有关。
excerpt_en: "A reading of OpenAI's Why Language Models Hallucinate, arguing that hallucination is tied to training paradigms and binary evaluation pressure, not only data noise or model flaws."
mathjax: true
...
大型语言模型（LLMs）的“幻觉”现象，是指模型生成过度自信但事实错误的陈述。从概率论和统计学习理论的角度来看：**幻觉不只是数据噪声或模型架构缺陷，也来自现代训练范式中的统计压力。** 论文证明，即使训练数据完全无噪声，交叉熵损失的最优化仍会导致不可避免的生成错误。幻觉在后训练阶段持续存在，还和评估激励有关：**当前主流评估机制（二元 0-1 评分）会系统性惩罚不确定性表达（如“我不知道”），从而奖励瞎猜行为。**
...
对于现实世界的许多情况，**计算学习理论已经给出了对应的 $\text{err}_{\text{iiv}}$ 下界**。因此在这些情况下，受制于上面的两个不等式，**语言模型的幻觉不可能完全消失：它们存在一个既定下界，这个下界取决于对应二分类问题的错误率。**
...
**结论**：论文统计指出，多数主流基准严格惩罚 IDK，导致模型在**评估体系中更倾向于猜测**。

### 缓解幻觉需要修改主流评分 Benchmark

现有主流评估基准大多采用二元评分方法。用于幻觉评估的 benchmark 虽然存在，但并不总是进入核心指标。想要缓解 LLM 生成幻觉，至少需要让核心 benchmark 把不确定性表达纳入评分设计。只要 LLM 还能在测试时依靠随机猜测获得更高评分，幻觉问题就很难解决：因为模型仍旧被奖励猜测，无法稳定学会回答 IDK。

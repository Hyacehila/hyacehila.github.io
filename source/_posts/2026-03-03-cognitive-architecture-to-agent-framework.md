...
tags: ["Tool Use", "Context Engineering", "Planning", "Multi-Agent"]
author: Hyacehila
...
我更愿意按一条实际的依赖关系讨论这些问题。CoALA 先给出记忆、规划、行动与环境的坐标；Workflow、Supervisor 和 Agent Team 再把控制权放到不同位置；然后讨论 Single Agent 与 MAS 的边界；框架最后把这些选择冻结成状态、节点、消息和恢复机制，以及他们如何变成技术债务以及我心中更好的 Agent Framework 应该是什么样子。顺序反过来，很容易先学会某个框架的词，再误以为那就是 Agent 本身。
...
### 三个早期案例：浏览器、软件团队与模拟小镇

WebGPT、MetaGPT 和 Generative Agents 经常一起出现在 Agent 入门材料中，但它们解决的并不是同一种问题。放到前面的认知结构里看，差异会清楚很多。

#### WebGPT：让模型在浏览器环境里找证据

[WebGPT](https://arxiv.org/abs/2112.09332) 把语言模型放进一个文本浏览器中，让它搜索、打开网页、引用来源，再根据收集到的材料回答问题。它更接近单体 Agent：一个模型维护当前任务状态，浏览器提供外部动作空间，搜索结果作为 observation 返回。答案生成和证据收集因此进入了同一条可训练、可评价的轨迹。

#### MetaGPT：用角色和 SOP 组织软件任务

[MetaGPT](https://arxiv.org/abs/2308.00352) 把产品经理、架构师、工程师等角色放进软件开发流程，并用结构化产物和标准作业流程约束协作。它常被归到多智能体系统，但从本文的分类看，更像高度结构化的 Agent Team，甚至接近伪 MAS：角色很多，通信也存在，系统主要依靠预先设计的流程、文档接口和阶段顺序保持收敛。

#### Generative Agents：记忆、反思和计划驱动的社会模拟

有些中文材料把 [Generative Agents](https://arxiv.org/abs/2304.03442) 概括成“西部世界沙盒”。论文实际构造了一个受《模拟人生》启发的小镇环境，25 个生成式智能体会记录经历、形成高层反思、生成日程并彼此互动。模拟能够持续运行，依赖的是每个角色各自的记忆流和计划；其他角色则构成环境的一部分。

这三个案例正好落在不同位置。WebGPT 展示单体 Agent 如何通过工具闭环收集证据；MetaGPT 展示 Workflow 和角色分工如何压住协作发散；Generative Agents 则把独立记忆、计划和消息交互放进同一个模拟环境。它们都叫 Agent，但控制权、记忆归属和通信边界完全不同。
...
通信本身也会成为系统设计的一部分。协议规定消息格式和可执行动作，拓扑决定点对点、广播还是中心调度，内容边界则决定共享结论、证据还是完整轨迹。协作、协商和竞争不再只是 prompt 里的角色台词，它们会改变权限、状态和后续行动。

...
title_en: "Re0-02: HuggingFace TRL SFTTrainer"
date: 2025-12-28 22:00:00 +0800
categories: ["Foundation Models", "Training & Alignment"]
tags: ["Hugging Face", "TRL", "Fine-Tuning", "Loss Masking", "Tutorial", "Series"]
author: Hyacehila
hidden: true
excerpt: 详解使用 TRL SFTTrainer微调LLM：Loss Masking、Flash-Attention
excerpt_en: "A detailed guide to fine-tuning LLMs with TRL SFTTrainer, covering loss masking and Flash-Attention."
permalink: '/blog/2025/12/28/Re0HF-02/'
---


这篇文章中的问题也可以和[Re0-01 : HuggingFace Transformers Trainer](/blog/2025/12/27/Re0HF-01/)、[Re0-03 : HuggingFace TRL DPOTrainer](/blog/2025/12/29/Re0HF-03/)放在一起阅读，以比较相近的概念如何在不同语境中展开。

> 配套代码：`sfttrainer.py`

## 承上启下

上一章用原生 `Trainer` 跑通了 SFT 训练。这个方案功能完整，但仍需要：
...
**本章介绍 TRL 库的 SFTTrainer**——面向 SFT 以及 RL 的训练器，用封装减少手写训练细节。
...
### 5.3 自定义 template 示例
自定义 template 的做法是在原始 template 的基础上，使用 generation 和 endgeneration 关键字包裹助手生成的部分，从而让 SFTTrainer 能够通过这个关键字进行掩码。

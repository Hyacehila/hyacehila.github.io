...
合成数据降低了写样本的成本，也更容易规模化复制同一种错误。更完整的数据合成流程见[《数据合成正在成为一门工程：从 Terminal-Corpus 说起》](/blog/2026/07/03/sft-synthetic-data-engineering/)。那篇文章讨论的是怎样从任务世界、环境和 verifier 反推训练数据；这里关心的是数据进入训练器后怎样被编码和计算 loss。

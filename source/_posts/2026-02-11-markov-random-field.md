...
tags: ["Markov Random Fields", "Graphical Models", "Spatial Statistics", "Gibbs Distribution"]
author: Hyacehila
...
在这一系列博客的开篇，我们讨论了**贝叶斯网络**，它使用**有向无环图 (DAG)** 来建模变量间的因果依赖。但在许多现实场景中，变量之间的作用是相互的、对称的，不存在明显的“因”与“果”。

例如，在数字图像中，一个像素及其相邻像素通常具有相似的颜色。我们不能说左边的像素“导致”了右边的像素，它们之间只是**空间相关**。对于这类问题，**无向图模型 (Undirected Graphical Models)**，即**马尔可夫随机场 (Markov Random Fields, MRF)**，是更自然的建模工具。
...
在无向图中，判断独立性比有向图直接，不再需要考虑 V-结构。这里使用**图分离 (Graph Separation)** 的概念：
...
为了定量描述分布 $P(X)$，可以利用图中的**团 (Clique)** 定义局部相互作用。
...
*   $x_C$ 是团 $C$ 中变量的状态配置。
*   $\psi_C(x_C) \ge 0$ 是**势函数 (Potential Function)** / **因子 (Factor)**。它是非负实函数，用于衡量团内变量状态的“相容性” (Compatibility)。
...
这建立了概率图模型与统计物理的联系。能量越低的状态，概率越高。

## 典型模型：Ising 模型与图像去噪

**伊辛模型 (Ising Model)** 起源于统计力学，用于模拟铁磁物质的磁偶极矩。它是最简洁的成对马尔可夫随机场 (Pairwise MRF)。
...
如果系统有 $N$ 个二值变量，求和项的数量高达 $2^N$。对于 $N=1000$ 的图像（仅 30x30 像素），计算量已超越宇宙原子总数。这就是 **Partition Function Intractability**。

### 蒙特卡洛马尔可夫链 (MCMC)

为了避开计算 $Z$，通常采用 **MCMC 采样** 方法。其思想是构造一个马尔可夫链，使其平稳分布收敛于目标分布 $P(X)$。

**吉布斯采样 (Gibbs Sampling)** 是常用方法。
它的优势在于：虽然计算联合概率 $P(X)$ 很难（因为有 $Z$），但计算**完全条件概率 (Full Conditional Probability)** 相对简单。
...
马尔可夫随机场通过**极大团势函数**的乘积定义联合概率分布，适合具有空间对称性的问题。Hammersley-Clifford 定理保证了这种因子分解的合法性。虽然配分函数 $Z$ 的存在使精确推断变得棘手，但 MCMC 等近似方法提供了可行的计算路径。

从贝叶斯网络的“因果”到 MRF 的“关联”，我们已经覆盖了概率图模型的两大基石。在下一篇（也是本系列的终篇），我们将探讨一种特殊的生成式有向图模型——**LDA 主题模型**，看它如何利用共轭先验处理文本数据挖掘问题。

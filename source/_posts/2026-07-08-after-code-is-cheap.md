...
过去，一个想法是否值得实现，常常会先被开发成本筛一遍。即使 PRD 已经写好，会议里也讲出了完整愿景，真正把它变成 MVP，仍然需要几周甚至几个月的开发、测试和协作。很多想法不是被证明没有价值，而是还没走到验证阶段，就先停在了愿望清单里。

到了 2026 年，Coding Agent 开始改写这道筛选机制。我们可能还没完全想清楚问题，它已经写出第一版实现、补上测试，并把 Demo 跑了起来。从 Idea 到 Code，原来要几周的链路可以缩到几小时。

变化不只是开发变快。过去稀缺的代码开始过剩，当实现不再是主要瓶颈，问题就落在了别处：我们该生产什么，又凭什么相信它？
...
在代码之前，先定义意图和边界。不要让 Agent 的第一份实现替我们决定问题的范围。写清用户、目标、约束、不能破坏的行为，以及什么结果能够证明这次修改有效。做原型时，也要提前写下继续和停止的信号。

在代码之中，限制生成规模，也挑战它的假设。把 AI 生成的代码当成一份未经信任的 PR，而不是答案。阅读它、运行它、寻找反例，并要求它解释为什么选择当前方案。更小的任务、更窄的 Context 和更短的反馈循环，通常比一次生成一万行代码更可靠。
...
“Talk is cheap. Show me the code.” 属于一个代码生产昂贵的时代。那时，能把想法变成代码，本身就是很有力的证明。

现在，一个能运行的版本可以在很短时间内生成，十种不同的实现也可以同时出现。代码不再足以证明理解、投入和质量。还得继续追问：它解决了什么问题？我们如何知道它是正确的？它在什么条件下会失败？谁理解它，又由谁承担结果？

所以这句话在今天可以改成：

> Code is cheap. Show me the evidence.

Evidence 背后仍然需要 Judgment，也需要一个愿意承担结果的 Owner。软件工程不会因为 Code is Cheap 而结束。问题定义、取舍、验证、沟通、维护、教学和责任，过去常被 Coding 的可见产出遮住，现在又回到了工程工作的前面。

After Code is Cheap，我们要做的是判断哪些代码值得存在，并为它们进入现实世界之后发生的一切负责。

...
将经典统计学方法迁移到空间数据，需要单独讨论。经过多年研究，空间数据已经形成了自身的理论体系。**整个空间数据分析体系基于空间自相关性**
...
#### 点数据
点数据(point data)  关注空间位置，不涉及属性值信息，比如居民点的空间分布、疫情爆发点的空间分布
...
空间数据类型可以相互转化，用来表达不同问题。Fothringham 将连续数据分析中的 Kriging 模型和多边形数据中的 SAR 等方法整合到一个体系中。我们也可以将原本多边形数据中的发病数量弱化为是否发病并转化为点数据，或构造区域等值线转换为连续数据，借此换用不同方法分析
...
地理信息系统（geographical information systems GIS） 是一种用于空间数据存储、展示、管理、查询、分析和决策支持的系统。其特点是处理的数据都经过地理编码，并将这种编码作为信息检索与处理的一部分。GIS 已经有了诸多专门软件，不需要局限于传统统计分析软件。而**大多数GIS也集成了常用的空间数据分析的功能**。
...
空间上相对接近的属性值，往往比相距更远的属性值更加相似。这被称为 Tobler 地理学第一定律，也是空间自相关的体现
万物世界空间分布不均 这就是空间异质性
...
空间自相关并不只有坏处。它让[[空间数据分析#空间插值]]成为可能；同时，空间回归模型也可以直接利用这种空间依赖性，改进预测效果

空间自相关性的解释需要结合指标值本身和相关领域知识，通常需要领域专家参与
...
空间分层异质性是空间异质性表现出的一种规律性。异质性本身也是地理学的基础：几乎每一个地点都存在相对于其他位置的独特性。
...
- 如果均显著，采用MSN SPA等模型
## 空间抽样
空间抽样是获取统计推断样本的方法。经典统计学研究过各种抽样方法，但面对空间数据的自相关性与分异性时，这些方法不再完全适用。因此需要单独讨论空间统计学中的抽样

在大数据时代，空间抽样乃至整个抽样调查体系的地位有所下降，因为我们更有能力进行完整抽样，因此这里从简介绍
...
### 空间系统抽样
系统抽样的基本思想是按固定间隔抽取单元；空间系统抽样则是把已有点尽量均匀地分散到二维空间中
...
### 空间Sandwich抽样
目前的抽样方法多针对报告单元进行抽样，原则上抽样数目至少应为报告单元数目的两倍，每个报告单元都要至少被抽到一次。这样不利于压缩样本量和控制抽样成本，于是提出了空间Sandwich抽样
...
## 空间插值
空间插值是空间统计学中的一个重要部分，可以由已知点推算大量未知位置的取值。

经典统计学中也会使用插值，但实际应用并不广泛；空间插值的应用更常见，它用少量样本值推算更大范围上的空间属性值，实践中很有用
...
### 趋势面插值
趋势面插值的思想很直接：利用某个已知函数拟合整体分布，函数的基本形式提前确定，只需估计其中的未知参数

趋势面插值严重依赖趋势面的选取；低次趋势面往往拟合效果堪忧，高次趋势面则计算负担较重
...
地理探测器以空间分层异质性为研究对象，基本思想是： **只要自变量对因变量有影响，那么空间分布应该存在一致性** 这里的空间可以是地理空间 属性空间 时间分类等多种变量
...
### 生态探测
对于第二个问题，可以通过构造F统计量比较两个自变量之间哪个更重要

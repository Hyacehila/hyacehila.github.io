...
excerpt_en: "A discussion of why statistics drifted away from applied settings while pursuing theoretical completeness, and how data science and machine learning reconnect with learning from data."
...
本文希望围绕“统计学的应用”与“应用的统计学”这两个看似纠缠的概念，讨论统计学为什么会在追求数学严谨性的道路上远离应用现场，以及现代数据科学与机器学习如何通过“数据驱动”的方式，把问题重新拉回现实约束中。
...
统计学诞生之初（如 Fisher 时期），无论是为了分析农业实验数据还是生物遗传特征，其目的都是为了**解决具体的应用问题**。随着学科的发展，统计学家们开始构建越来越完整的理论体系。为了让体系在数学上完备，各种精妙的假设被引入，理论推导也越来越繁复、精致。

最终，统计学也形成了类似数学的塔状结构：处于顶层的统计学家不再处理具体的数据，而是研究抽象的分布族、渐进性质和最优性证明。**应用的统计学逐渐异化为了统计学的应用**——即拿着锤子找钉子，强行将现实问题削足适履，以套用那些完美的理论模型。

原本面向应用的学科，逐渐与应用现场拉开距离；对应的学生培养也逐渐偏离应用问题，最终难以培养出能解决应用问题的人才。
...
Breiman 敏锐地指出，统计学不应以成为二流数学家为目标，而应培养**收集信息、分析信息并得出结论的专家**。如果统计学家不能在语音处理、天文、医学等具体应用中找到乐趣并解决问题，那么统计学的身份危机将永远无法解决。
...
报告中最鲜明的观点是**实践（Practice）必须重新成为评价统计学工作的中心**。长久以来，统计学界存在一种错误的二元对立：将“理论统计学”视为高贵的上层建筑，而将“应用统计学”视为次要的体力劳动。这种价值观导致了学科的异化：学者们为了获得终身教职（Tenure），不得不通过发表晦涩难懂的数学理论来证明自己的智力，而对解决科学或社会问题缺乏兴趣。

这份迟到的报告呼吁：评价一个统计学家的标准，不应再是数学技巧有多繁复，而应是其对解决实际问题的**影响力（Impact）**。
...
此时，如果我们依然坚持**先假设，再推断**的传统范式，就会发现：真实世界的数据几乎从未完整满足这些通过数学推导出来的完美假设。
...
但应用统计学在现实中并没有因此停摆。许多时候，统计模型之所以还能用相关关系去说明某种“因果”，并不是因为模型从数据中自行识别出了因果结构，而是因为**因果关系早已内嵌在研究人员对问题的理解、研究设计与变量选择之中**。研究者往往并不是要从数据里“发现因果”，而是先基于领域知识对什么可能影响什么形成判断，再借助统计模型去估计强度、比较方向、量化不确定性。严格说，这并不是完全由数据推出的因果识别，却正是今天许多应用统计研究的现实：模型并不创造因果解释，它只是在尝试把研究者已经带入分析中的因果理解参数化、数量化。

## 应用的统计学：数据科学与机器学习的接棒

既然传统的理论统计学在应用面前碰了壁，那么是谁接过了应用的统计学这一棒呢？

是**数据科学（Data Science or Data Mining）**与**机器学习（Machine Learning）**。

### 殊途同归：从 Inference 到 Prediction

人们常问：统计学和机器学习有什么区别？
一种精辟的回答是：**它们都绕不开同一个问题：我们如何从数据中学习？**

但它们侧重点的差异，正好解释了为什么机器学习在大数据时代大放异彩：
* 统计学：强调统计推断（Inference）。关注置信区间、假设检验、参数的估计。它试图解释模型（Explainability），为此不得不对数据分布做出强假设（如逻辑回归）。
* 机器学习：强调预测（Prediction）。它将数据生成机制视为一个黑盒，不强求理解内部的参数含义，只求 $f(x)$ 能最准确地预测 $y$。

在高维、结构繁杂的现实问题中（如图像识别、推荐系统），人类根本无法预设正确的数学分布形式。机器学习摒弃了对形式完美的追求，通过交叉验证（Cross-Validation）等经验主义手段来评估模型的好坏，而非依赖理论上的渐进正态性。

但直到今天，仍有不少统计学家以及一些社会科学研究者，将“解释”视为更高贵、更科学的目标，而把“预测”贬低为一种不够学术、只关心工程效果的技术活动。这种看法本身就是站不住脚的。**解释与预测并不是科学与非科学的对立，而是科学研究中两种同样重要的目标。** 解释帮助我们理解机制、组织知识、形成理论；预测帮助我们检验模型是否抓住了稳定结构，帮助我们在未知样本、未来情境与现实决策中衡量一个方法到底有没有用。一个只能解释却无法在新数据上表现稳定的模型，很难说真正把握了世界；而一个能够持续做出准确预测的模型，也绝不能轻易被说成“不科学”。

这其实是在回到“应用的统计学”的初衷：**方法首先要能解决问题。**

### 第四范式的崛起

数据科学的诞生，正是为了应对这种数据维度的爆炸和来源的复杂化。作为科学研究的**第四范式（数据密集型科学发现）**，数据科学不再依赖于理论推演或计算仿真所需的先验知识，而是直接从数据出发，发现模式。

它不是简单的学科叠加，而是几类能力的深度融合：

* **计算机科学**提供了基础设施：从核心的数据库技术（存储与提取）到云计算与分布式系统，计算能力的飞跃使得海量数据的处理成为可能。
* **统计学**提供了方法论的演进：
  * 计算统计学（如 MCMC、EM 算法）取代了部分传统解析解，有力支撑了复杂统计结构的处理。
  * 探索性数据分析（EDA）在大数据时代重新焕发光彩，帮助我们从数据海洋中“嗅探”信息。
  * 高维统计学结合压缩感知与稀疏性处理，为“维度诅咒”提供了数学上的解药。
* **人工智能**提供了强大的工具：从传统的模式识别进化到现代机器学习，再到深度学习的爆发，提供了自动化特征提取与非线性建模的强大能力。
...
* 转向工业界：处理最末端的现实世界应用。在这里，我们可以进一步细分为两个紧密联系但侧重点不同的角色：
  * 数据科学家（Data Scientist, DS）：侧重于从数据中提取见解（Insights），通过分析帮助企业或组织做出科学决策。他们更接近于“咨询者”和“发现者”。
  * 机器学习工程师（Machine Learning Engineer, MLE）：侧重于构建产品（Products）。他们的首要任务是将算法模型工程化、落地化，转化为实际可用的服务。

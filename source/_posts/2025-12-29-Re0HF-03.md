...
title_en: "Re0-03: HuggingFace TRL DPOTrainer"
date: 2025-12-29 22:00:00 +0800
categories: ["Foundation Models", "Training & Alignment"]
tags: ["Hugging Face", "TRL", "Fine-Tuning", "Alignment", "Tutorial", "Series"]
author: Hyacehila
hidden: true
excerpt: 详解使用 TRL DPOTrainer微调LLM：实现简单的偏好对齐
excerpt_en: "A detailed guide to fine-tuning LLMs with TRL DPOTrainer and implementing simple preference alignment."
permalink: '/blog/2025/12/29/Re0HF-03/'
---

> 配套代码：`dpotrainer.py`

这篇文章中的问题也可以和[Re0-02 : HuggingFace TRL SFTTrainer](/blog/2025/12/28/Re0HF-02/)、[Re0-01 : HuggingFace Transformers Trainer](/blog/2025/12/27/Re0HF-01/)放在一起阅读，以比较相近的概念如何在不同语境中展开。
...
**但是**，SFT 的局限在于：它让模型学习"如何回答"，却不直接区分"什么样的回答更好"。

**本章介绍 DPO（Direct Preference Optimization）**——一种让模型学习人类偏好的训练方法。若要系统理解 DPO 的数学理论，仍应阅读原文；下面只保留必要推导。
...
DPO 的主要思路是：**如果我们知道了最优策略的奖励函数形式，就可以绕过奖励模型，直接优化策略。**
...
**主要优势**：DPO 将强化学习问题转化为了**监督学习问题**，不需要训练奖励模型，不需要复杂的 RL 算法，代码实现接近 SFT。
...
DPO 的重点在于如何将强化学习对齐转换为损失函数优化，因此有很多衍生形式，可以通过这里进行选择。
```python
...
- [TRL 文档：DPOTrainer](https://huggingface.co/docs/trl/main/en/dpo_trainer)

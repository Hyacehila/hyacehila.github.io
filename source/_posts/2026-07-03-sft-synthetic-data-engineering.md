...
这篇论文的系统叫 Terminal-Task-Gen，产出的数据叫 Terminal-Corpus，最后用这些 SFT 轨迹训练 Nemotron-Terminal。它把任务生成、环境、teacher rollout、过滤、课程学习、长上下文和规模实验放在一起讨论。读起来舒服，是因为它没有只给一个结果，而是把训练数据管线拆开给人看：哪些数据从旧数据集改造而来，哪些任务按技能合成，teacher 怎么跑，失败样本怎么处理，训练时怎么混。
...
定义任务世界，生成可验证任务，收集可学习轨迹，维护可追踪的数据管线。prompt 仍然重要，但它只是入口。训练效果取决于这套世界能不能稳定地产生、筛选和回流那些会改变模型行为的数据。还得有一套评估，证明这些工作确实有效。
...
在看 NVIDIA 做的这些工程之前，先回头看几条前面的路线：怎样启动任务池，怎样控制复杂度，怎样让数据更可学习，怎样把生成过程拆成可检查的流水线。

### 自举任务池：Self-Instruct

[Self-Instruct](https://arxiv.org/abs/2212.10560) 解决的是起点问题。没有足够人工 instruction 时，它从 175 条人工 seed tasks 出发，让模型自己扩任务池。每一轮生成时，它抽 8 条 instruction 作为上下文，其中 6 条来自人工种子，2 条来自模型之前生成的数据。模型先生成新 instruction，再判断是否符合需求，最后生成 input 和 output。
...
第二组工作更接近现在的数据策展实践：怎样让数据覆盖目标范围，怎样实现动态的难度控制与课程学习，以及怎样理解错误样本和数据规模的价值。不同研究给出的答案并不完全一致。
...
[OpenCodeReasoning](https://arxiv.org/abs/2504.01943) 则重新思考错误样本的价值。它收集 TACO、APPS、CodeContests 和 CodeForces 等问题得到 28,904 个独特题目，再用 DeepSeek-R1 生成 736,712 条 Python reasoning samples。但实验结果有些让人意外：过滤掉未通过单元测试的样本后，模型性能反而下降了。这不能直接说明错误样本本身有益，更可能说明过滤同时改变了题目难度、覆盖范围或轨迹分布。一个可能的解释是失败样本覆盖了更多困难题，这个结果和 NV 的 trajectory filtering 呼应：正确性是重要信号，但不等于训练价值的全部。

[CHIMERA](https://arxiv.org/abs/2603.00889) 是一个规模上的反提醒。它只构造约 9,225 条 compact synthetic reasoning data，用于跨科学领域推理。流程是先从粗学科扩展到 8 个学科、1,179 个 topics，再生成自包含且答案可验证的问题，用多个模型交叉验证，最后用 thinking model 合成解题轨迹，保留正确轨迹做 SFT。它说明小数据也可以有价值，但前提是覆盖密度和验证质量足够清楚。CHIMERA 和 OpenCodeReasoning 并不能被简单归纳为相反观点：前者说明经过组织和验证的小规模数据也能有效，后者则提醒我们，激进的正确性过滤可能连同困难样本和有用覆盖一起删除。过滤与数据重组本身也会引入 bias，因此每一条清洗规则都需要说明理由，并检查它改变了怎样的数据分布。

## 回到 Terminal-Corpus：数据合成工程

现在回到 NVIDIA 这篇文章。Terminal-Task-Gen 不是靠一个大 prompt 批量生成题目，而是先把数据来源、任务格式、运行环境、teacher 行为和后续过滤摆在同一条管线上。terminal 与环境密切相关，但与环境密切相关的不止 terminal。Role-Play、QA、RAG-Sys、Chatbot 都有自己的环境，只是藏在聊天记录和先验的世界知识里。
...
特别是采用这种数据合成思路时，我们务必要维护足够完整的 Schema，以及 `task_id`、来源、难度、生成版本等任务信息。不同来源需要消融，不同难度也需要消融，因此应该在合成阶段尽可能完整地记录元数据。只保存 `(instruction, response)` 和一张简单表格，早期省事，后期排查时会非常痛苦。
...
两条 synthetic generation 最后都会落到同一种任务格式：自然语言 task prompt、pytest-based tests、带权重的测试项与部分得分、补充输入文件，以及一个 domain-specific Docker environment。论文没有为每个任务生成标准答案，它转而追求一种更工程化的目标：任务可以很难解，但验证要尽量清楚。
...
在 1%、5%、10%、100% 的数据比例上观察趋势，可以先用较小的数据比例获得初步判断。如果 10% 的数据就接近 100% 的效果，可能说明当前评估下的覆盖已经接近饱和，继续增加同分布数据的边际收益有限。这个发现本身就有工程价值。这是对数据规模的消融，是很经典的训练消融技巧，但不是全部。

另外，论文用 8B 模型完成主要消融，再用 14B 和 32B 模型验证规模趋势，降低结论只在一个模型尺寸上成立的可能性。同时，我们还可以观察类似的 scaling 趋势是否会在自己的场景中出现。
...
在论文里，负面的消融不用藏着。你只需要解释"为什么这个结果是 counter-intuitive 但合理的"——比如课程学习失败是因为数据来源排序不等于难度排序，严格过滤有害是因为困难任务的恢复行为也被一起删掉了。消融实验本来就不一定阳性，negative results 也会有价值。在本文这类预算和实验设置下，可以用 8B 模型完成主要消融，再用其他模型尺寸做 scaling experiments；这是一种资源分配策略，不是对所有研究都成立的规则。

## 先建立评估基准

读完这些工作以后，我不会一开始就追求十万条数据。更舒服的起点是先把评估和验证的架子搭起来，先知道怎么判断自己做对了，再谈怎么造数据。这仍然是数据工程主线的一部分：没有可信的评估，数据生成管线就拿不到稳定反馈。验证自己的工作和造数据，在真实论文里本来就不该分开。

这一步最容易被跳过，却决定后面所有实验的可信度。没有 benchmark 就开始训练，连自己在优化什么都说不清。
...
用一个强模型（GPT-5 / Claude Opus）做自动评分可以大幅降低评估成本，但必须经过校准才能可信。MT-Bench 展示了 GPT-4 judge 与人类偏好之间较高的一致性，但这个结果依赖具体任务、样本和评测协议，不能直接迁移到新的研究场景。如果不能在自己的数据上报告 judge 与人类的一致性，审稿人有充分理由质疑 LLM-as-judge 的可靠性。
...
- **Position bias**：judge 可能偏好某个固定位置的回答。可以将 A/B 两个回答交换位置各评一次，并单独报告交换前后的稳定性。
- **Verbosity bias**：judge 可能把更长、更完整的表述误判为质量更高，需要用长度匹配样本或人工标注集检查这种倾向。
- **Self-enhancement bias**：judge 可能偏好与自身模型家族相近的输出，因此最好使用多个 judge 或加入跨模型、人工校准。

#### 多信号三角验证

当你的 benchmark 规模小、case study 案例少、LLM-as-judge 校准不足时，单一信号无法支撑任何结论。但如果 benchmark 分数提升 + case study 显示行为变化 + LLM-as-judge 给出相同方向的评分趋势，三个弱信号合在一起就更有说服力。审稿人真正反感的是用一个不可靠的指标提出过强的主张——让主张强度和证据强度匹配，这是发表策略里最常被忽略的原则。
...
数据工程是否有效，最终还需要通过实验设计和证据链来说明。这里继续讨论怎样把整个流程从一个实验变成一篇论文：工业界上线可以依赖 A/B Test 和长期运行指标，论文则需要用可复现的实验、基线和消融从其他角度说服审稿人。
...
合成数据是一套训练世界的维护工作。prompt、teacher、verifier 和 schema 都重要。它正在变成一项更系统的工程，对普通研究者反而是好消息：只要问题认识得足够清楚，从小规模数据开始也能积累真实的工程经验。

...
定义任务世界，生成可验证任务，收集可学习轨迹，维护可追踪的数据管线。prompt 仍然重要，但它只是入口。真正决定训练效果的，是这套世界能不能稳定地产生、筛选和回流那些会改变模型行为的数据。除此之外，我们还需要建立一套能够证明这些工作有效的评估方法。
...
读完这些工作以后，我不会一开始就追求十万条数据。更舒服的起点是先把评估和验证的架子搭起来——先知道怎么判断自己做对了，再谈怎么造数据。这仍然是数据工程主线的一部分：没有可信的评估，数据生成管线就无法获得稳定的反馈。下面这条路径把"怎么验证自己的工作"和"怎么造数据"放在一起，因为它们在真实的论文里本来就不该分开。
...
合成数据是一套训练世界的维护工作。prompt、teacher、verifier 和 schema 都非常重要。数据合成正在成为一项更系统的工程，这对普通研究者反而是好消息：只要对问题的认识足够清楚，从小规模数据开始也能积累真实的工程经验。
